...
function replace_star_ratings($html, $rating, $count)
{
	...
	if (0 < $count) {
		...
		$html .= sprintf(_n('%1$s based on %2$s rating', 'Rated %1$s based on %2$s ratings', $count, 'woocommerce'), '<strong class="rating">' . esc_html($rating) . '</strong>', '<span class="rating">' . esc_html($count) . '</span>');
	} else {
