...
class EH_Bancontact extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_description = sprintf( __( 'Bancontact allow customer to use Bancontact card or mobile app linked to a Belgian bank account to make online payments. <a class="thickbox" href="%sassets/img/bancontact-preview.png?TB_iframe=true&width=100&height=100">[Preview] </a> ', 'payment-gateway-stripe-and-woocommerce-integration' ), EH_STRIPE_MAIN_URL_PATH );
        $this->supports = array(
            'products',
            'refunds',
        );
        ...
        $this->title                   = $this->get_option( 'eh_stripe_bancontact_title' );
        $this->description             = $this->get_option( 'eh_stripe_bancontact_description' );
    ...
    public function payment_scripts() {
        ...
        if ( (is_checkout()  && !is_order_received_page())) {
         ...
         wp_enqueue_script('eh_bancontact_js', plugins_url('assets/js/eh-bancontact.js', EH_STRIPE_MAIN_FILE), array('stripe_v3_js','jquery'),EH_STRIPE_VERSION, true);
            ...
            $stripe_params['is_checkout'] = ( is_checkout() && !isset( $_GET['pay_for_order']  ) ) ? 'yes' : 'no';
    ...
    public function payment_fields() {
        ...
        if ($description) {
            echo wp_kses_post(apply_filters('eh_bancontact_desc', wpautop(wp_kses_post("<span>" . $description . "</span>"))));
        }
        echo "</div>";
    ...
    public function process_payment($order_id) { 
        ...
        try{ 
            ...
            $payment_method = isset($_POST['eh_bancontact_token']) ? sanitize_text_field(wp_unslash($_POST['eh_bancontact_token'])) : '';
