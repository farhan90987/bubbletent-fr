...
/**
 * Class for performing reference transaction related transactions requests via the PayPal Express Checkout API,
 * including the creation of a billing agreement and processing renewal payments using that billing
 * agremeent's ID in a reference tranasction.
 */
class SA_Buy_Now_WC_PayPal_Reference_Transaction_API extends SA_Buy_Now_WC_SV_API_Base {
	...
	/**
	 * URL-decoded and parsed parameters
	 *
	 ...
	 *
	 * @param array    $paypal_args Paypal arguments for transaction request.
	 * @param WC_Order $order WooCommerce order object.
	 * @return array
	 * @since 2.0
	 */
	public function get_paypal_args( $paypal_args = array(), $order = object ) {
	...
	public function are_reference_transactions_enabled() {
		...
		$this->set_response_handler( 'SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response' );
		...
		try {
			...
			if ( ! $response->has_api_error() ) {
				...
				if ( $response->has_api_error_for_credentials() ) {
					update_option( 'sa_bn_wc_paypal_credentials_error', 'yes', 'no' );
				}
			}
		} catch ( Exception $e ) {
			$reference_transactions_enabled = false;
	 ...
	 * @return SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Checkout response object
	 * @since 2.0
	 */
	public function set_express_checkout( $args = array() ) {

		$request = $this->get_new_request();

		$request->set_express_checkout( $args );

		$this->set_response_handler( 'SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Checkout' );
	 ...
	 * @return SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Billing_Agreement response object
	 * @since 2.0
	 */
	public function create_billing_agreement( $token = '' ) {
		...
		$this->set_response_handler( 'SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Billing_Agreement' );
	 ...
	 * @param array    $args arguments for express checkout request.
	 * @return SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Payment refund response
	 * @since 2.0.9
	 */
	public function do_express_checkout( $token = '', $order = object, $args = array() ) {
		...
		$this->set_response_handler( 'SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Payment' );
	 ...
	 * @param string   $reference_id billing agreement id.
	 * @param WC_Order $order order object.
	 ...
	 * @since 2.0
	 */
	public function do_reference_transaction( $reference_id = '', $order = object, $args = array() ) {
		...
		// This class handles prefix issue in do reference transaction request since they do not have any prefix as compared to do express checkout response.
		$this->set_response_handler( 'SA_Buy_Now_WC_PayPal_Do_Reference_Transaction_API_Response_Payment' );
	 ...
	 * @since 2.0.0
	 * @see SV_WC_Payment_Gateway_API::refund()
	 * @param string   $profile_id billing agreement id.
	 * @param string   $new_status new profile status.
	 * @param WC_Order $order order object.
	 * @return SV_WC_Payment_Gateway_API_Response refund response
	 * @throws SV_WC_Payment_Gateway_Exception Network timeouts, etc.
	 */
	public function manage_recurring_payments_profile_status( $profile_id = '', $new_status = '', $order = object ) {
	...
	public function get_callback_url( $action = '' ) {
		return add_query_arg( 'action', $action, WC()->api_request_url( 'sa_bn_wc_paypal' ) );
	}
	...
	protected function get_new_request( $args = array() ) {
		return new SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Request( $this->api_username, $this->api_password, $this->api_signature, self::VERSION );
	}
	...
	protected function get_plugin() {
		return SA_Buy_Now_WC_Paypal_Standard::instance();
	}
}
