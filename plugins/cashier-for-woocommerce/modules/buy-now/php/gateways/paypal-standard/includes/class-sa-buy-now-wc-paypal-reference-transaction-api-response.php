...
/**
 * Class for parsing response string received from the PayPal Express Checkout API for Reference Transaction related requests, which is simply a URL-encoded string of parameters.
 */
class SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response extends WC_Gateway_Paypal_Response {
	...
	public function has_api_error_for_credentials() {
		...
		if ( $this->has_api_error() ) {
			...
			foreach ( range( 0, 9 ) as $index ) {
				...
				$has_credentials_error_code    = $this->has_parameter( "L_ERRORCODE{$index}" ) && in_array( $this->get_parameter( "L_ERRORCODE{$index}" ), array( 10002, 10008 ), true );
				$has_credentials_error_message = $this->has_parameter( "L_LONGMESSAGE{$index}" ) && in_array( $this->get_parameter( "L_LONGMESSAGE{$index}" ), array( 'Username/Password is incorrect', 'Security header is not valid' ), true );
	...
	public function get_api_error_message() {
		...
		foreach ( range( 0, 9 ) as $index ) {
			...
			if ( $this->has_parameter( "L_SHORTMESSAGE{$index}" ) ) {
				...
				$error_message = sprintf(
					'%s: %s - %s',
					$this->has_parameter( "L_SEVERITYCODE{$index}" ) ? $this->get_parameter( "L_SEVERITYCODE{$index}" ) : _x( 'Error', 'used in api error message if there is no severity code from PayPal', 'cashier' ),
					$this->get_parameter( "L_SHORTMESSAGE{$index}" ),
					$this->has_parameter( "L_LONGMESSAGE{$index}" ) ? $this->get_parameter( "L_LONGMESSAGE{$index}" ) : _x( 'Unknown error', 'used in api error message if there is no long message', 'cashier' )
				);
		...
		return empty( $error_messages ) ? _x( 'N/A', 'no information about something', 'cashier' ) : trim( implode( ', ', $error_messages ) );
	}
	...
	public function get_user_message() {
		...
		return in_array( $this->get_api_error_code(), $allowed_user_error_message_codes, true ) ? $this->get_api_error_message() : null;
	}
	...
	public function to_string() {
		...
		return print_r( $this->parameters, true ); // phpcs:ignore
	}
