...
/**
 * Class for handling responses from PayPal IPN for Reference Transactions.
 */
class SA_Buy_Now_WC_PayPal_Reference_Transaction_IPN_Handler extends SA_Buy_Now_WC_PayPal_Standard_IPN_Handler {
	 ...
	 *
	 * @param  array $transaction_details posted transaction details from PayPal ipn request.
	 */
	public function valid_response( $transaction_details = array() ) {
		...
		switch ( $transaction_details['txn_type'] ) {
			...
			case 'merch_pmt':
				if ( ! empty( $transaction_details['custom'] ) ) {

					$order = $this->get_paypal_order( $transaction_details['custom'] );
					if ( true === sa_bn_wc_is_order( $order ) ) {
						...
						WC_Gateway_Paypal::log( 'Found order #' . sa_bn_wc_get_objects_property( $order, 'id' ) );
	 ...
	 *
	 * @param string $billing_agreement_id billing agreement id for subscription.
	 * @since 2.0
	 */
	protected function cancel_subscriptions( $billing_agreement_id = '' ) {
		$note = esc_html__( 'Billing agreement cancelled at PayPal.', 'cashier' );

		foreach ( SA_Buy_Now_WC_Paypal_Standard::get_orders_by_paypal_id( $billing_agreement_id, 'objects' ) as $subscription ) {
			...
			if ( $is_paypal_subscription && ! $subscription->has_status( sa_bn_wc_get_subscription_ended_statuses() ) ) {
				try {
