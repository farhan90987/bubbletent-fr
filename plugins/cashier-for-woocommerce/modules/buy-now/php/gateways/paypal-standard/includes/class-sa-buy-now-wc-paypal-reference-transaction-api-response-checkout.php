...
/**
 * Class for Express Checkout API calls.
 */
class SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Checkout extends SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response {
	...
	private function get_state_code( $country_code = '', $state = '' ) {
		...
		if ( 'US' !== $country_code && isset( WC()->countries->states[ $country_code ] ) ) {
			...
			if ( ! empty( $local_states ) && in_array( $state, $local_states, true ) ) {
