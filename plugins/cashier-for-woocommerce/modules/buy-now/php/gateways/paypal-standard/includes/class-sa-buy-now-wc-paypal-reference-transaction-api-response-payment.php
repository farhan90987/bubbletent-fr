...
/**
 * Class for parsing DoExpressCheckout response which are used to process initial payments (if any) when checking out.
 */
class SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Payment extends SA_Buy_Now_WC_PayPal_Reference_Transaction_API_Response_Billing_Agreement {
	...
	public function transaction_approved() {
		...
		return in_array( $this->get_payment_status(), $this->successful_statuses, true );
	}
	...
	public function get_status_message() {
		...
		if ( $this->transaction_held() ) {
			...
			$message = sprintf( __( 'expected clearing date %s', 'cashier' ), date_i18n( wc_date_format(), sa_bn_wc_date_to_time( $this->get_payment_parameter( 'EXPECTEDECHECKCLEARDATE' ) ) ) );
		}

		// add fraud filters.
		$filters = $this->get_fraud_filters();
	...
	public function get_avs_result() {
		...
		if ( is_array( $filters ) && ! empty( $filters ) ) {
			...
			foreach ( $filters as $filter ) {
				...
				if ( in_array( $filter['id'], range( 1, 3 ), true ) ) {
	 ...
	 *
	 * A wrapper around @see SA_Buy_Now_WC_Reference_Transaction_Checkout_API_Response::has_parameter()
	 ...
	 *
	 * A wrapper around @see SA_Buy_Now_WC_Reference_Transaction_Checkout_API_Response::get_parameter()
	 * that prepends the @see self::get_payment_parameter_prefix().
	 *
	 * @since 2.0.9
	 * @param string $name parameter name.
