...
/**
 * Class handling IPN requests from PayPal for PayPal Standard Subscription transactions
 */
class SA_Buy_Now_WC_PayPal_Standard_IPN_Handler extends WC_Gateway_Paypal_IPN_Handler {
	 ...
	 *
	 * @param bool   $sandbox is sandbox environment.
	 * @param string $receiver_email reciver email.
	 */
	 ...
	 * @since 2.0
	 */
	public function valid_response( $transaction_details = array() ) {
		global $wpdb;
	 ...
	 *
	 * @param array $transaction_details Post data after wp_unslash.
	 * @throws Exception Fatal errors during processing ipn request.
	 * @since 2.0
	 */
	protected function process_ipn_request( $transaction_details = array() ) {
		...
		if ( empty( $subscription ) ) {
			...
			if ( in_array( $transaction_details['txn_type'], array( 'subscr_cancel', 'subscr_eot' ), true ) ) {
		...
		if ( isset( $transaction_details['txn_id'] ) ) {
			...
			if ( in_array( $ipn_transaction_id, $handled_transactions, true ) ) {
		...
		if ( in_array( $transaction_details['txn_type'], array( 'subscr_signup', 'subscr_payment' ), true ) && false !== strpos( $transaction_details['invoice'], '-wcsfrp-' ) ) {
			...
			if ( sa_bn_wc_get_objects_property( $transaction_order, 'id' ) !== get_post_meta( $subscription->get_id(), '_paypal_failed_sign_up_recorded', true ) ) {
		...
		if ( $is_renewal_sign_up_after_failure || $is_payment_change ) {
			...
			$existing_profile_id = sa_bn_wc_get_paypal_id( $subscription );
		...
		if ( isset( $transaction_details['subscr_id'] ) && ! in_array( $transaction_details['txn_type'], array( 'subscr_cancel', 'subscr_eot' ), true ) ) {
			sa_bn_wc_set_paypal_id( $subscription, $transaction_details['subscr_id'] );

			if ( sa_bn_wc_is_paypal_profile_a( $transaction_details['subscr_id'], 'out_of_date_id' ) && 'disabled' !== get_option( 'sa_bn_wc_paypal_invalid_profile_id' ) ) {
				update_option( 'sa_bn_wc_paypal_invalid_profile_id', 'yes', 'no' );
			}
		}
		...
		switch ( $transaction_details['txn_type'] ) {
			case 'subscr_signup':
				...
				if ( $is_payment_change ) {
					...
					$this->add_order_note( _x( 'IPN subscription payment method changed to PayPal.', 'when it is a payment change, and there is a subscr_signup message, this will be a confirmation message that PayPal accepted it being the new payment method', 'cashier' ), $subscription, $transaction_details );

				} else {

					$this->add_order_note( __( 'IPN subscription sign up completed.', 'cashier' ), $subscription, $transaction_details );
			...
			case 'subscr_payment':
				...
				if ( 'completed' === strtolower( $transaction_details['payment_status'] ) ) {
					...
					$this->add_order_note( __( 'IPN subscription payment completed.', 'cashier' ), $subscription, $transaction_details );
					...
					if ( $is_first_payment ) {
					...
					} elseif ( $subscription->get_completed_payment_count() === 1 && '' !== SA_Buy_Now_WC_Paypal_Standard::get_option( 'identity_token' ) && 'true' !== get_post_meta( $subscription->get_id(), '_paypal_first_ipn_ignored_for_pdt', true ) && false === $is_renewal_sign_up_after_failure ) {
						...
						if ( true === $is_renewal_sign_up_after_failure && is_object( $transaction_order ) ) {
							...
							update_post_meta( $subscription->get_id(), '_paypal_failed_sign_up_recorded', sa_bn_wc_get_objects_property( $transaction_order, 'id' ) );
							...
							if ( 'paypal' === get_post_meta( $subscription->get_id(), '_old_payment_method', true ) ) {
								...
								$this->add_order_note( __( 'IPN subscription failing payment method changed.', 'cashier' ), $subscription, $transaction_details );
							}
						}
						...
						$this->add_order_note( __( 'IPN subscription payment completed.', 'cashier' ), $transaction_order, $transaction_details );

						add_action( 'woocommerce_subscription_activated_paypal', 'WCS_PayPal_Status_Manager::reactivate_subscription' );

						sa_bn_wc_set_paypal_id( $transaction_order, $transaction_details['subscr_id'] );
					}
				} elseif ( in_array( strtolower( $transaction_details['payment_status'] ), array( 'pending', 'failed' ), true ) ) {

					// Subscription Payment completed.
					// translators: placeholder is payment status (e.g. "completed").
					$this->add_order_note( sprintf( _x( 'IPN subscription payment %s.', 'used in order note', 'cashier' ), $transaction_details['payment_status'] ), $subscription, $transaction_details );

					if ( ! $is_first_payment ) {

						sa_bn_wc_set_objects_property( $transaction_order, 'transaction_id', $transaction_details['txn_id'] );

						if ( 'failed' === strtolower( $transaction_details['payment_status'] ) ) {
							$subscription->payment_failed();
							// translators: placeholder is payment status (e.g. "completed").
							$this->add_order_note( sprintf( _x( 'IPN subscription payment %s.', 'used in order note', 'cashier' ), $transaction_details['payment_status'] ), $transaction_order, $transaction_details );
						} else {
							$transaction_order->update_status( 'on-hold' );
							// translators: placeholder is payment status (e.g. "completed").
							$this->add_order_note( sprintf( _x( 'IPN subscription payment %1$s for reason: %2$s.', 'used in order note', 'cashier' ), $transaction_details['payment_status'], $transaction_details['pending_reason'] ), $transaction_order, $transaction_details );
						}
					}
			...
			case 'recurring_payment_suspended':
				...
				if ( sa_bn_wc_get_paypal_id( $subscription ) !== $ipn_profile_id ) {
					...
					$subscription->update_status( 'on-hold', __( 'IPN subscription suspended.', 'cashier' ) );
			...
			case 'subscr_cancel':
				...
				if ( sa_bn_wc_get_paypal_id( $subscription ) !== $transaction_details['subscr_id'] ) {
					...
					$subscription->cancel_order( __( 'IPN subscription cancelled.', 'cashier' ) );
			...
			case 'recurring_payment_suspended_due_to_max_failed_payment':
				$ipn_failure_note = __( 'IPN subscription payment failure.', 'cashier' );
	 ...
	 * @param string $meta_key   PayPal payload data.
	 * @since 2.0
	 */
	public static function get_order_id_and_key( $args = array(), $order_type = 'shop_order', $meta_key = '_paypal_subscription_id' ) {
		...
		if ( empty( $order_id ) && isset( $args['custom'] ) ) {
			...
			if ( is_object( $order_details ) ) { // WC 2.3.11+ converted the custom value to JSON, if we have an object, we've got valid JSON.
				...
				if ( 'shop_order' === $order_type ) {
					...
					// Subscription created with Subscriptions < 2.0.
					$subscriptions = sa_bn_wc_get_subscriptions_for_order( absint( $order_details->order_id ), array( 'order_type' => array( 'parent' ) ) );
				...
				WC_Gateway_Paypal::log( __( 'Invalid PayPal IPN Payload: unable to find matching subscription.', 'cashier' ) );
			}
		}
	 ...
	 */
	protected static function get_parent_order_with_fallback( $subscription = object ) {
	 ...
	 * @since 2.0
	 */
	protected static function cancel_subscription( $subscription = object, $old_paypal_subscriber_id = '' ) {
		...
		$current_profile_id = sa_bn_wc_get_paypal_id( $subscription->get_id() );
	...
	protected function validate_transaction_type( $txn_type = '' ) {
		if ( in_array( strtolower( $txn_type ), $this->get_transaction_types(), true ) ) {
			return true;
		} else {
			return false;
		}
	 ...
	 * @since 2.0.20
	 */
	protected function add_order_note( $note = '', $order = object, $transaction_details = array() ) {
		$note = apply_filters( 'sa_bn_wc_paypal_ipn_note', $note, $order, $transaction_details );
		if ( ! empty( $note ) ) {
	 ...
	 *
	 * @param WC_Subscription object $subscription subscription object.
	 ...
	 * @since 2.4.3
	 */
	protected function get_order_by_transaction_id( $subscription = object, $transaction_id = 0, $order_types = 'any' ) {
	 ...
	 * @since 2.1
	 */
	protected function get_renewal_order_by_transaction_id( $subscription = object, $transaction_id = 0 ) {
	 ...
	 * @since 2.4.3
	 */
	protected function get_parent_order_by_transaction_id( $subscription = object, $transaction_id = 0 ) {
