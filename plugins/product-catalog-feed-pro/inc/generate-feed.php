...
class WPwoofGenerator {
	...
	private function excludeChilds( $excluded, $isTags ) {
		...
		if ( $isadded ) {
			$this->excludeChilds( $excluded, $isTags );
		}
	}
...
//                    $this->_storedebug("feed_filter_price_bigger".$parent_prices . " => " . print_r($parent_prices, true),true );
			...
			$this->_feed_settings_for_save['status_feed'] = 'error: ' . $e->getMessage();

			wpwoof_update_feed( $this->_feed_settings_for_save, $this->_feed_id, true );

			update_option( "wpwoofeed_errors", $this->_feed_settings_for_save['status_feed'] );
