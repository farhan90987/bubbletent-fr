 ...
 */
namespace MarketPress\German_Market\Dompdf\Renderer;

use DOMElement;
use MarketPress\German_Market\Dompdf\Adapter\CPDF;
use MarketPress\German_Market\Dompdf\Css\Color;
...
abstract class AbstractRenderer
{
     ...
     * Rendering backend
     *
     * @var \MarketPress\German_Market\Dompdf\Canvas
     */
    protected $_canvas;
    /**
    ...
    protected function addHyperlink(DOMElement $node, array $borderBox): void
    {
        if ($node->nodeName === "a" && ($href = $node->getAttribute("href")) !== "") {
            [$x, $y, $w, $h] = $borderBox;
            $dompdf = $this->_dompdf;
            $href = Helpers::build_url($dompdf->getProtocol(), $dompdf->getBaseHost(), $dompdf->getBasePath(), $href, $dompdf->getOptions()->getChroot()) ?? $href;
