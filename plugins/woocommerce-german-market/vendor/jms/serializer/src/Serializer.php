...
use MarketPress\German_Market\JMS\Serializer\ContextFactory\DefaultDeserializationContextFactory;
use MarketPress\German_Market\JMS\Serializer\ContextFactory\DefaultSerializationContextFactory;
...
use MarketPress\German_Market\JMS\Serializer\Exception\UnsupportedFormatException;
...
final class Serializer implements SerializerInterface, ArrayTransformerInterface
{
    ...
    private function getNavigator(int $direction): GraphNavigatorInterface
    {
        if (!isset($this->graphNavigators[$direction])) {
            throw new RuntimeException(sprintf('Can not find a graph navigator for the direction "%s".', GraphNavigatorInterface::DIRECTION_SERIALIZATION === $direction ? 'serialization' : 'deserialization'));
        }
    ...
    {
        $factories = GraphNavigatorInterface::DIRECTION_SERIALIZATION === $direction ? $this->serializationVisitors : $this->deserializationVisitors;
        if (!isset($factories[$format])) {
            throw new UnsupportedFormatException(sprintf('The format "%s" is not supported for %s.', $format, GraphNavigatorInterface::DIRECTION_SERIALIZATION === $direction ? 'serialization' : 'deserialization'));
        }
