    ...
    function __construct() {
		...
		$this->customer_can_edit_payment_information = $this->get_option( 'my_account_customer_can_edit_payment_information', 'off' );
		 ...
		 if ( self::$instances == 1 ) {
			...
			// don't show instructions in "confirm" email
			if ( 'on' === get_option( 'woocommerce_de_manual_order_confirmation' ) ) {
				add_action( 'wgm_manual_order_confirmation_after_remove_all_actions', function( $order_id ) {
					remove_action( 'woocommerce_email_before_order_table', array( $this, 'email_instructions' ), 10, 3 );
				});

				add_action( 'wgm_manual_order_confirmation_confirm_after_send_email', function() {
					add_action( 'woocommerce_email_before_order_table', array( $this, 'email_instructions' ), 10, 3 );
				});
			}
	...
	function init_form_fields() {
		...
		if ( is_admin() ) {
			...
			if ( isset( WC()->countries ) && WGM_Helper::method_exists( WC()->countries, 'get_allowed_countries' ) ) {
				if ( isset( $_REQUEST[ 'section' ] ) && 'german_market_sepa_direct_debit' === $_REQUEST[ 'section' ] ) {
