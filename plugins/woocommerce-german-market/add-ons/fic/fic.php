...
if ( ! function_exists( 'german_market_fic_init' ) ) {
	...
	*/
	function german_market_fic_init() {
		...
		if ( is_admin() || apply_filters( 'german_market_fic_init_admin_in_frontend', false ) ) {
		...
		if ( version_compare( WC_VERSION, '8.7.0', '>=' ) ) {
			require_once( $backend_dir . DIRECTORY_SEPARATOR . 'edit-new-product-editor.php' );
			add_action( 'woocommerce_block_template_area_product-form_after_add_block_general', array( 'German_Market_FIC_Product_Editor', 'add_fic_data' ) );
		}
