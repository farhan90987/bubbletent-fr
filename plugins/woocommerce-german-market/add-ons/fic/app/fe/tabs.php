...
function gm_fic_have_to_add_tabs( $product ) {
	...
	if ( $product->get_type() == 'variable' ) {
		...
		foreach ( $children as $child ) {
			...
			if ( ! $add_allergens_info ) {
				...
				if ( '' !== $allergens_info && is_string( $allergens_info ) ) {
			...
			if ( ! $add_ingredients ) {
				...
				if ( '' !== $ingredients && is_string( $ingredients ) ) {
	...
	if ( ! $add_allergens_info ) {
		$allergens_info = get_post_meta( $id, '_allergens_info', true );
	 	$add_allergens_info = '' !== $allergens_info && is_string( $allergens_info );
	}

 	// ingredients
 	if ( ! $add_ingredients ) {
	 	$ingredients = get_post_meta( $id, '_fic_ingredients', true );
	 	$add_ingredients = '' !== $ingredients && is_string( $ingredients );
	}
...
function gm_fic_tab_content_nutritional_values() {
	...
	if ( apply_filters( 'gm_fic_ui_frontend_display_h2', true, 'nutritional_values' ) ) {
		?>
		<h2><?php echo esc_attr( get_option( 'gm_fic_ui_frontend_labels_nutritional_values', __( 'Nutritional Values', 'woocommerce-german-market' ) ) ); ?></h2>
	<?php } ?>
...
function gm_fic_tab_content_allergens() {
	...
	if ( apply_filters( 'gm_fic_ui_frontend_display_h2', true, 'allergens' ) ) {
		?>
		<h2><?php echo esc_attr( get_option( 'gm_fic_ui_frontend_labels_allergens', __( 'Allergens', 'woocommerce-german-market' ) ) ); ?></h2>
...
function gm_fic_tab_content_ingredients() {
	...
	if ( apply_filters( 'gm_fic_ui_frontend_display_h2', true, 'ingredients' ) ) {
		?>
		<h2><?php echo esc_attr( get_option( 'gm_fic_ui_frontend_labels_ingredients', __( 'Ingredients', 'woocommerce-german-market' ) ) ); ?></h2>
...
function gm_fic_tab_content_nutritional_values_by_id( $id ) {
	...
	?>
	<span class="gm_fic_nutritional_values_remark"><?php echo wp_kses_post( strval( apply_filters( 'gm_fic_nutritional_values_remark', get_post_meta( $id, '_nutritional_values_remark', true ) ) ) ); ?></span>
		...
		foreach ( $terms as $term ) {
			...
			$post_value = apply_filters( 'gm_fic_nutritional_values_post_value', $post_value, $post_key, $term->slug, $id );
			...
			$required = apply_filters( 'gm_fic_nutritional_values_is_term_required', $required, $term, $default_nutritionals, $id );

			if ( ! $required && $post_value == '' ) {
				continue;
			}

			$label = apply_filters( 'gm_fic_nutritional_values_term_name', $term->name, $term );
			$class = 'nut-' . $term->slug;
			...
			?>
			<tr class="gm-fic-nutritional-values-tr <?php echo esc_attr( $class ); ?>">
				<td class="gm-fic-nutritional-values-td gm-fic-nutritional-values-td-label <?php echo esc_attr( $class ); ?>"><?php echo wp_kses_post( $label ); ?></td>
				<td class="gm-fic-nutritional-values-td gm-fic-nutritional-values-td-value <?php echo esc_attr( $class ); ?>"><?php echo wp_kses_post( strval( $post_value ) ); ?></td>
...
function gm_fic_tab_content_allergens_by_id( $id ) {
	...
	if ( is_string( $allergens_info ) ) {

		do_action( 'gm_fic_tab_content_allergens_by_id_before_content' );

		echo wp_kses_post( apply_filters( 'gm_fic_tab_content_allergens_by_id_content', apply_filters( 'the_content', $allergens_info ), $allergens_info ) );
...
function gm_fic_tab_content_ingredients_by_id( $id ) {
	...
	if ( is_string( $ingredients ) ) {
		...
		echo wp_kses_post( apply_filters( 'gm_fic_tab_content_ingredients_by_id_content', apply_filters( 'the_content', $ingredients ), $ingredients ) );
...
function gm_fic_product_tab_scripts() {
	...
	$in_footer = 'on' === get_option( 'german_market_frontend_js_in_footer', 'off' );
	wp_register_script( 'gm-fic-frontend', untrailingslashit( plugin_dir_url( __FILE__ ) ) . '/assets/js/frontend.' . $min . 'js', array( 'jquery' ), Woocommerce_German_Market::$version, $in_footer );
