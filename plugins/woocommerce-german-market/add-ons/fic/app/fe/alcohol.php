...
function gm_fic_alcohol_get_markup( $value, $unit ) {
	...
	if ( '' === $unit ) {
		if ( apply_filters( 'gm_fic_alcohol_use_default_unit_if_unit_is_empty', true ) ) {
			$unit = get_option( 'gm_fic_ui_alocohol_default_unit', __( '% vol', 'woocommerce-german-market' ) );
		}
	}
...
function gm_fic_woocommerce_add_order_item_meta_wc_3( $item_id, $item, $order_id ) {
	...
	if ( is_a( $item, 'WC_Order_Item_Product' ) ) {
		...
		if ( is_string( $alcohol_value ) && is_string( $alcohol_unit ) ) {
