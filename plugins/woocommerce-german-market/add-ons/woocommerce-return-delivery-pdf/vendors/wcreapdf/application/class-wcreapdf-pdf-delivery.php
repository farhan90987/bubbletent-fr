...
if ( ! class_exists( 'WCREAPDF_Pdf_Delivery' ) ) {
	...
	if ( ! defined( 'GM_EURO' ) ) {
		define( 'GM_EURO' , WCREAPDF_Helper::utf8_encode( chr( 128 ) ) );
	}
	...
	* @category	Class
	*/
	class WCREAPDF_Pdf_Delivery extends MarketPress_German_Market_FPDF {
		...
		function Header() {
			...
			if ( has_action( 'wcreapdf_pdf_header_delivery' ) ) {
				...
				if ( $print_page_numbers ) {
					$this->Cell( 0, $header_cell_height, WCREAPDF_Helper::utf8_decode( $string_page ) . ' '. $this->PageNo(). ' / ' . $all_pages, 0, 0, 'R' );
				}
				// reset the margin
		...
		*/
		function Footer() {
			...
			$footer_text 		= str_replace( '•', WCREAPDF_Helper::utf8_encode( chr( '183' ) ), $footer_text );
			...
			if ( has_action( 'wcreapdf_pdf_footer_delivery' ) ) {
				...
				$this->MultiCell( 0, $footer_cell_height, WCREAPDF_Helper::utf8_decode( $footer_text . '' ), 0, $footer_text_align );
			}
		}
		...
		*/
		public static function create_pdf( $order = NULL, $test = false, $download = false, $for_zip = false, $admin = false ) {
			...
			if ( has_action( 'wcreapdf_pdf_custom_fonts' ) && ( ! in_array( $font, array( 'Helvetica', 'Courier', 'Times' ) ) ) ) {
				if ( 'default_cache_path' === get_option( 'wcreapdf_cache_fontpath', 'default_cache_path' ) ) {
					...
					define( 'FPDF_FONTPATH', get_option( 'wcreapdf_cache_fontpath' ) );
				}
			}
			...
			if ( has_action( 'wcreapdf_pdf_iamge_bind_or_shop_name_delivery' ) ) {
				...
				if ( ! $image_bind ) {                                                              // if it wasn't possible to insert image, output shopname
					...
					$pdf->Cell( 0, $cell_big, WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_shop_name_delivery' ), get_bloginfo( 'name' ) ) ), 0, 0, 'L' ); // output shopname
				}
			}
			...
			if ( has_action( 'wcreapdf_pdf_addresses_delivery' ) ) {
				...
				if ( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_shop_adress_position_delivery', 'right' ) ) == 'right' ) {
					...
					$pdf->Cell( $address_empty_space_x, $cell_small, WCREAPDF_Helper::utf8_decode( $string_recipient . ':' ), 0, 0, 'L' );   // output 'Recipient' string
					$pdf->Cell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( $string_consignor . ':' ), 0, 0, 'L' );
					$pdf->Ln( $br2 );
					...
					$pdf->MultiCell( $address_empty_space_x, $cell_small, WCREAPDF_Helper::utf8_decode( html_entity_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_address_delivery' ) ) ) ), 0, 'L' );	// output shop address
					...
					foreach ( $addressArray as $addressRow ) {                              // output shipping address
						$pdf->Cell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( html_entity_decode( trim( $addressRow ), ENT_QUOTES ) ), 0, 1, 'L' );
					}
					...
					$new_consignor = array();
					foreach ( $addressArray as $addressRow ) {                              // output shipping address
						$new_consignor[] = WCREAPDF_Helper::utf8_decode( html_entity_decode( trim( $addressRow ) ) );
					}
					...
					$addressArray   = explode( '<br/>', $formattedAddress );
					foreach ( $addressArray as $addressRow ) {                              // output shipping address
						$pdf->Cell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( html_entity_decode( trim( $addressRow ) ) ), 0, 1, 'L' );
					}
			...
			if ( has_action( 'wcreapdf_pdf_return_delivery_delivery' ) ) {
				...
				$pdf->Cell( 0, $cell_huge, WCREAPDF_Helper::utf8_decode( $string_return_delivery ) . ":", 0, 1, 'L', 0 ); // output 'Return delivery'
				// order number
				...
				if ( ! $test ) {
					...
					$replace = array( WCREAPDF_Helper::utf8_decode( rand( 100, 9999 ) ), date_i18n( get_option( 'date_format' ), current_time( 'timestamp' ) ) );
				}
				...
				$pdf->Cell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( $small_headline ), 0, 1, 'L' );
			}
			...
			if ( has_action( 'wcreapdf_pdf_table_header_delivery' ) ) {
				...
				$pdf->Cell( $column1b,  $cell_small,    WCREAPDF_Helper::utf8_decode( $string_amount ),  0, 0, 'L', 0 );				// output amount

				if (  get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'weight_delivery' ), true ) ) {
					$pdf->Cell( $column1c,  $cell_small,    WCREAPDF_Helper::utf8_decode( $string_weight ),  0, 0, 'L', 0 );			// output weight
				} else {
					$column1c = 0;
				}

				if (  get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'sku_delivery' ), true ) ) {
					$pdf->Cell( $column1,   $cell_small, WCREAPDF_Helper::utf8_decode( $string_suk ),        0, 0, 'L', 0 );			// output suk
				} else {
					$column1 = 0;
				}

				$pdf->Cell( $column2, 	$cell_small, WCREAPDF_Helper::utf8_decode( $string_product ),	0, 1, 'L', 0 );				// output product
			...
			if ( has_action( 'wcreapdf_pdf_table_content_delivery' ) ) {
				...
				foreach ( $items as $item_id => $item ) {
					...
					if ( ! $test ) {
						...
						$name 						  = str_replace( '„', WCREAPDF_Helper::utf8_encode( chr( 132 ) ), $name );
						$name 						  = str_replace( '“', WCREAPDF_Helper::utf8_encode( chr( 147 ) ), $name );
						$name 						  = str_replace( '‘', WCREAPDF_Helper::utf8_encode( chr( 145 ) ), $name );
						$name 						  = str_replace( '’', WCREAPDF_Helper::utf8_encode( chr( 146 ) ), $name );
						...
						$item_meta_display_string = apply_filters( 'wcrepdf_delivery_item_meta_display_string', html_entity_decode( $item_meta_display_string, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 ), $item, $order );

						if ( WGM_Helper::method_exists( $_product, 'get_weight' ) && WGM_Helper::method_exists( $item, 'get_quantity' ) ) {
							if ( $_product->get_weight() != '' ) {
								if ( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'quantity_refund_delivery' ), 'exclude' ) == 'exclude' ) {
									$weight = floatval( $_product->get_weight() ) * $item->get_quantity();
									$weight = apply_filters( 'wcrepdf_delivery_item_weight_output', $weight, $_product->get_weight(), $item->get_quantity() );
									$total_weight += $weight;
								} else {
									$weight = floatval( $_product->get_weight() ) * ( $item->get_quantity() + $order->get_qty_refunded_for_item( $item_id ) );
					...
					$pdf->Cell( $column1b, $cell_small, WCREAPDF_Helper::utf8_decode( $quantity ), 0, 0, 'L' );                     	// output amount

					if (  get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'weight_delivery' ), true ) ) {
						$pdf->Cell( $column1c,  $cell_small, WCREAPDF_Helper::utf8_decode( $weight ),  0, 0, 'L', 0 );   		  	// output weight
					}
					...
					if (  get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'sku_delivery' ), true ) ) {		// output suk
						$current_y = $pdf->GetY();
						$pdf->MultiCell( ( $column1 - 5 ), $cell_small, WCREAPDF_Helper::utf8_decode( $sku ), 0, 'L', 0 );
					...
					$current_y = $pdf->GetY();
					$pdf->MultiCell( $column2, $cell_small, WCREAPDF_Helper::utf8_decode( $name ), 0, 'L', 0 );                     	// output product name
					...
					if ( $item_meta_display_string != '' ) {                                                       // output variation meta data
						...
						$pdf->MultiCell( $column2, $cell_tiny, WCREAPDF_Helper::utf8_decode( $item_meta_display_string ), 0, 'L', 0 );
					...
					if ( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'show_short_description_delivery' ), false ) ) {
						...
						if ( $test ) {
							...
							if ( is_object( $_product ) && method_exists( $_product, 'get_id' ) ) {
								...
								$short_description = str_replace( '<br>', PHP_EOL, $short_description );
								$short_description = strip_tags( $short_description );
								$short_description = apply_filters( 'wcreapdf_pdf_short_description', $short_description, $_product, $order );
							}
						}
						...
						if ( ! empty( $short_description ) ) {
							...
							$pdf->MultiCell( $column2, $cell_tiny, WCREAPDF_Helper::utf8_decode( $short_description ), 0, 'L', 0 );
							$pdf->SetFont( $font, $bold10, $small );
						}
			...
			if ( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'show_delivery_date' ), false ) ) {
				if ( has_action( 'wcreapdf_pdf_delivery_date' ) ) {
					...
					if ( apply_filters( 'wcreapdf_pdf_delivery_date_show_if_completed', $is_completed ) ) {
						...
						$pdf->Cell( 0, $cell_big, WCREAPDF_Helper::utf8_decode( $string_delivery_date ) . ":", 0, 1, 'L', 0 );          // output 'Delivery Date'
						...
						$pdf->MultiCell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( html_entity_decode( $delivery_date ) ), 0, 'L', 0 );
			...
			if ( has_action( 'wcreapdf_pdf_remark_delivery' ) ) {
				...
				if ( $remark != '' ) { // no remark => no remark output at all
					...
					if ( ! $test ) {
						...
						$order_number = WCREAPDF_Helper::utf8_decode( rand( 100, 9999 ) );
					...
					$pdf->SetFont( $font, $bold12, $big );
					$pdf->Cell( 0, $cell_big, WCREAPDF_Helper::utf8_decode( $string_remark ) . ":", 0, 1, 'L', 0 );                 // output 'Remark'
					$pdf->SetFont( $font, $bold13, $small );
					$pdf->MultiCell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( html_entity_decode( $remark ) ), 0, 'L', 0 );    // output remark text that has been saved in backend
					$did_remark = true;
				}
			}
			...
			if ( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_show_customers_note' ), 'off' ) == 'on' ) {
				...
				if ( ! $test ) {
					...
					if ( has_action( 'wcreapdf_pdf_customer_note_delivery' ) ) {
						do_action( 'wcreapdf_pdf_customer_note_delivery', $pdf, $variables, $order, $test, $download );
					} else {
						...
						if ( ! empty( trim( $customer_note ) ) ) {
							...
							$pdf->SetFont( $font, $bold12, $big );
							$pdf->Cell( 0, $cell_big, WCREAPDF_Helper::utf8_decode( $string_customer_note ), 0, 1, 'L', 0 );
							$pdf->SetFont( $font, $bold13, $small );
							$pdf->MultiCell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( html_entity_decode( $customer_note ) ), 0, 'L', 0 );
						}
					}
				}
			}
			...
			if ( ! empty( $after_pdf ) ) {
				$pdf->Ln( $br6 );
				$pdf->SetFont( $font, $bold13, $small );
				$pdf->MultiCell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( $after_pdf ), 0, 'L', 0 );
			}
		...
		*/
		public function set_pdf_author_title() {
			$this->SetAuthor( WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_author_delivery' ), get_bloginfo( 'name' ) ) ), false );
			$this->SetTitle( WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_title_delivery' ), __( 'Delivery Note', 'woocommerce-german-market' ) . ' - ' . get_bloginfo( 'name' ) ) ) );
			$this->SetSubject( WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_title_delivery' ), __( 'Delivery Note', 'woocommerce-german-market' ) . ' - ' . get_bloginfo( 'name' ) ) ) );
		}
		...
		*/
		public function wc_output( $order, $download, $test, $for_zip = false, $admin = false ) {
			...
			do_action( 'wcreapdf_pdf_before_output', 'delivery', $order, $admin, $this );
			$used_for_file_name = true;
			...
			if ( $download === false ) {
				...
				if ( $for_zip ) {
					...
					$filename = get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_file_name_delivery' ), __( 'Delivery-Note', 'woocommerce-german-market' ) );
					$filename = apply_filters( 'wcreapdf_temp_filename_delivery', $filename, $order );
					$this->Output( WCREAPDF_TEMP_DIR . 'pdf' .  DIRECTORY_SEPARATOR . $directory_name .  DIRECTORY_SEPARATOR . $filename . '.pdf', "F" );
					do_action( 'wcreapdf_pdf_after_output', 'delivery', $order );
				...
				$download_string = apply_filters( 'wcreapdf_download_destination_delivery', $download_string );
				$suffix = ( $test === false ) ? '-' . $order->get_order_number() : '';
				$filename = WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_file_name_delivery' ), __( 'Delivery-Note', 'woocommerce-german-market' ) ), $used_for_file_name ) . $suffix;
				$filename = apply_filters( 'wcreapdf_download_filename_delivery', $filename, $order );
				$this->Output( $download_string, $filename . '.pdf' );
				do_action( 'wcreapdf_pdf_after_output', 'delivery', $order );
