...
if ( ! class_exists( 'WCREAPDF_Pdf' ) ) {
	...
	* @category	Class
	*/
	class WCREAPDF_Pdf extends MarketPress_German_Market_FPDF {
		...
		public static function create_pdf( $order = NULL, $test = false, $download = false, $for_zip = false, $admin = false ) {
			...
			if ( has_action( 'wcreapdf_pdf_iamge_bind_or_shop_name' ) ) {
				...
				if ( ! $image_bind ) { // if it wasn't possible to insert image, output shopname
					...
					$pdf->Cell( 0, $cell_big, WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_shop_name' ), get_bloginfo( 'name' ) ) ), 0, 0, 'L' ); // output shopname
				}
			}
			...
			if ( has_action( 'wcreapdf_pdf_addresses' ) ) {
				...
				$pdf->MultiCell( 0, $cell_small, WCREAPDF_Helper::utf8_decode( html_entity_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_address' ) ) ) ), 0, 'L' );	// output shop address
			...
			if ( has_action( 'wcreapdf_pdf_table_header' ) ) {
				...
				$pdf->Cell( $column2, 	$cell_small, WCREAPDF_Helper::utf8_decode( $string_product ),	0, 0, 'L', 0 );				// output product
				$pdf->Cell( $column4, 	$cell_small, '',                                0, 0, 'L', 0 );
				$pdf->Cell( $column3, 	$cell_small, WCREAPDF_Helper::utf8_decode( $string_quantity ), 	0, 0, 'C', 0 );				// output quantity
				$pdf->Cell( $column4, 	$cell_small, '',                                0, 0, 'L', 0 );
				$reasons_star	= ( $there_are_reasons ) ? $string_star : '';                                       // if so output '*' else ''
				$pdf->Cell( $column5, $cell_small, WCREAPDF_Helper::utf8_decode( $string_reason ) . $reasons_star, 0, 1, 'C', 0 );
			...
			if ( has_action( 'wcreapdf_pdf_table_content' ) ) {
				...
				foreach ( $items as $item_id => $item ) {
					...
					if ( ! $test ) {
						...
						$item_meta_display_string = apply_filters( 'wcrepdf_item_meta_display_string', html_entity_decode( $item_meta_display_string, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 ), $item, $order );
			...
			if ( has_action( 'wcreapdf_pdf_delivery_reasons' ) ) {
				...
				if ( $there_are_reasons ) {
					$pdf->Cell( $return_delivery_row_width, $cell_big, $string_star . WCREAPDF_Helper::utf8_decode( $string_delivery_reasons ), 0, 1, 'L' );
					...
					for ( $i = 0; $i <= ($first_column - 1 ); $i++ ) {         // output first column
						...
						$pdf->MultiCell( ( $delivery_reasons1 - $width_of_string_100 ), $cell_small, trim( WCREAPDF_Helper::utf8_decode( $reasons_array[ $i ] ) ), 0, 'L' );   // output reason
					}
					$pdf->SetY( $current_y2 );
					for ( $i = $first_column; $i <= ( $reasons_all - 1 ); $i++ ) { // output second column
						...
						$pdf->Cell( $width_of_string_100, $cell_small, $ordinal . '. ', 0, 0, 'R' );                              // outpout ordinal number
						$pdf->MultiCell( $delivery_reasons2 - $width_of_string_100, $cell_small, trim( WCREAPDF_Helper::utf8_decode( $reasons_array[ $i ] ) ), 0, 'L' ); // output reason
					}
				}
				$pdf->SetY( $current_y );
			}
			...
			if ( has_action( 'wcreapdf_pdf_comments' ) ) {
				...
				$pdf->Cell( 0, $cell_big, WCREAPDF_Helper::utf8_decode( $string_comments . ":" ) , 0, 1, 'L' );
				$pdf->SetFont( $font, $bold15, $small );
				$pdf->Ln( $br8 );
				// lines for comments
		...
		*/
		public function set_pdf_author_title() {
			$this->SetAuthor( WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_author' ), get_bloginfo( 'name' ) ) ), false );
			$this->SetTitle( WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_title' ), __( 'Retoure', 'woocommerce-german-market' ) . ' - ' . get_bloginfo( 'name' ) ) ) );
			$this->SetSubject( WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_title' ), __( 'Retoure', 'woocommerce-german-market' ) . ' - ' . get_bloginfo( 'name' ) ) ) );
		}
		...
		*/
		public function wc_output( $order, $download, $test, $for_zip = false, $admin = false ) {
			...
			do_action( 'wcreapdf_pdf_before_output', 'retoure', $order, $admin, $this );
			...
			if ( $download === false ) {
				...
				if ( $for_zip ) {
					...
					$filename = get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_file_name' ), __( 'Retoure', 'woocommerce-german-market' ) );
					$filename = apply_filters( 'wcreapdf_temp_filename', $filename, $order );
				...
				$download_string = apply_filters( 'wcreapdf_download_destination', $download_string );
				$suffix = ( $test === false ) ? '-' . $order->get_order_number() : '';
				$filename = WCREAPDF_Helper::utf8_decode( get_option( WCREAPDF_Helper::get_wcreapdf_optionname( 'pdf_file_name' ), __( 'Retoure', 'woocommerce-german-market' ) ), $used_for_file_name ) . $suffix;
				$filename = apply_filters( 'wcreapdf_download_filename', $filename, $order );
