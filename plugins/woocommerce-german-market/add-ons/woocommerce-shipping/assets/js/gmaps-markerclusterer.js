...
function MarkerClusterer( map, opt_markers, opt_options ) {
	 ...
	 * @private
	 */
	this.gridSize_ = (options[ 'gridSize' ] !== undefined) ?
		options[ 'gridSize' ] :
		60;
...
Cluster.prototype.addMarker = function( marker ) {
	...
	if ( ! this.center_ ) {
		...
		if ( this.averageCenter_ ) {
			...
			var lat = (this.center_.lat() * (l - 1) +
				marker.getPosition().lat()) / l;
			var lng = (this.center_.lng() * (l - 1) +
				marker.getPosition().lng()) / l;
...
ClusterIcon.prototype.triggerClusterMouseover = function( event ) {
	...
	google.maps.event.trigger( markerClusterer, 'clustermouseover',
		this.cluster_, event );

	if ( typeof this.cluster_.markerClusterer_.onMouseoverCluster_ ===
		'function' ) {
...
ClusterIcon.prototype.triggerClusterMouseout = function( event ) {
	...
	google.maps.event.trigger( markerClusterer, 'clustermouseout',
		this.cluster_, event );

	if ( typeof this.cluster_.markerClusterer_.onMouseoutCluster_ ===
		'function' ) {
...
function defaultClusterOnAdd( clusterIcon ) {
	...
	google.maps.event.addDomListener( clusterIcon.div_, 'click',
		function( event ) {
	...
	google.maps.event.addDomListener( clusterIcon.div_, 'mousedown',
		function() {
			isDragging = false;
			isMouseDown = true;
		} );
	...
	google.maps.event.addDomListener( clusterIcon.div_, 'mousemove',
		function() {
			if ( isMouseDown ) {
				isDragging = true;
			}
	...
	google.maps.event.addDomListener( clusterIcon.div_, 'mouseover',
		function( event ) {
			clusterIcon.triggerClusterMouseover( event );
		} );

	google.maps.event.addDomListener( clusterIcon.div_, 'mouseout',
		function( event ) {
...
ClusterIcon.prototype.getPosFromLatLng_ = function( latlng ) {
	...
	if ( typeof this.iconAnchor_ === 'object' && this.iconAnchor_.length ===
		2 ) {
...
ClusterIcon.prototype.onRemove = function() {
	if ( typeof this.cluster_.markerClusterer_.onRemoveCluster_ ===
		'function' ) {
...
ClusterIcon.prototype.createCss = function( pos ) {
	...
	if ( ! markerClusterer.cssClass_ ) {
		...
		if ( typeof this.anchor_ === 'object' ) {
			if ( typeof this.anchor_[ 0 ] === 'number' && this.anchor_[ 0 ] >
				0 &&
				this.anchor_[ 0 ] < this.height_ ) {
				style.push( 'height:' + (this.height_ - this.anchor_[ 0 ]) +
					'px; padding-top:' + this.anchor_[ 0 ] + 'px;' );
			} else if ( typeof this.anchor_[ 0 ] === 'number' &&
				this.anchor_[ 0 ] < 0 &&
				-this.anchor_[ 0 ] < this.height_ ) {
			...
			if ( typeof this.anchor_[ 1 ] === 'number' && this.anchor_[ 1 ] >
				0 &&
			...
			style.push( 'height:' + this.height_ + 'px; line-height:' +
				this.height_ + 'px; width:' + this.width_ +
...
ClusterIcon.prototype.addClass = function() {
	...
	if ( markerClusterer.cssClass_ ) {
		this.div_.className = markerClusterer.cssClass_ + ' ' +
			markerClusterer.cssDefaultClass_ + this.setIndex_;
	} else {
