(function( $, window, document ) {
	...
	function handle_delete_shipping_labels() {
		if ( $( '.wgm-shipping-buttons .delete' ).length ) {
			$( '.wgm-shipping-buttons .delete' ).on( 'click', function( event ) {
				event.preventDefault();
				let url = $( this ).attr( 'href' );
				$.get( url, function( response ) {
					if ( true === response.success ) {
						$( '.wgm-shipping-buttons' ).remove();
					}
				});
			});
		}
	...
	function handle_shipping_method_modal_input_fields( checked ) {
		if ( checked ) {
			$( 'input[name*=class_cost]' ).prop( 'readonly', true );
			if ( $( 'input[name*=class_cost]' ).length ) {
				$( 'select[id$=_type]' ).prop( 'disabled', true );
			}
			$( 'textarea[id$=_cost_rates]' ).prop( 'readonly', false );
		} else {
			$( 'input[name*=class_cost]' ).prop( 'readonly', false );
			if ( $( 'input[name*=class_cost]' ).length ) {
				$( 'select[id$=_type]' ).prop( 'disabled', false );
			}
			$( 'textarea[id$=_cost_rates]' ).prop( 'readonly', true );
		}
	}

	function init_shipping_method_modal_observer() {
		if ( $( '.wc-shipping-zone-methods' ).length ) {
			$( document.body ).on( 'wc_backbone_modal_loaded', function( target ) {
				if (
					$( '.wc-shipping-zone-methods' ).length &&
					$( 'input[id^=woocommerce_dhl_]' ).length &&
					$( 'input[id$=calc_weight_based]' ).length
				) {
					let checkbox = $( 'input[id$=calc_weight_based]' );
					handle_shipping_method_modal_input_fields( checkbox.is( ':checked' ) );
					$( checkbox ).on( 'change', function() {
						handle_shipping_method_modal_input_fields( $( this ).is( ':checked' ) );
					})
				} else
				if (
					$( 'select[id^=woocommerce_dhl_]' ).length &&
					$( 'select[id$=type]' ).length
				) {
					let select = $( 'select[id$=type]' );
					let option = $( 'select[id$=type] option:selected' ).val();
					handle_shipping_method_modal_input_fields( 'weight' === option );
	...
	$( function() {
		handle_delete_shipping_labels();
		init_shipping_method_modal_observer();
	} );
