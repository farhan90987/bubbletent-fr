...
*
*/
class WP_WC_Invoice_Pdf_Page_Numbers {

	public static function add_page_numbers( $pageNumber, $pageCount, $canvas, $fontMetrics ) {
        ...
        // page number output not desired
        if ( isset( $GLOBALS[ "hide_page_number_pages" ] ) ) {
        	if ( in_array( $pageNumber, $GLOBALS[ "hide_page_number_pages" ] ) ) {
        		return;
        	}
        }
	   ...
	   if ( $position != "none" ) {
			...
			// text
			$text			= $page_numbers_text;
			$pseudo_text	= str_replace( array( "{{current_page_number}}", "{{total_page_number}}" ), array( "9", "9" ), $text );
			$text			= str_replace( array( "{{current_page_number}}", "{{total_page_number}}" ), array( $pageNumber, $pageCount ), $text );
