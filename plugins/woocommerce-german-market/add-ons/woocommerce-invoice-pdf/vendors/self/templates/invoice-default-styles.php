...
<style>
	...
	.helper-billing-address {
		border: <?php echo ( ( $billing_addr_border_color == '' ) ? 'none' : ( 'solid ' . $billing_addr_border_px .'px ' . $billing_addr_border_color ) ); ?>;
	...
	table.items-table {
		border-top: <?php echo 'solid ' . $border_width . 'px ' . $border_color; ?>; 
		border-left: <?php echo 'solid ' . $border_width . 'px ' . $border_color; ?>;  
		border-right: <?php echo 'solid ' . $border_width . 'px ' . $border_color; ?>;  
	}
	
	table.totals-table {
		border-left: <?php echo 'solid ' . $border_width . 'px ' . $border_color; ?>;  
		border-right: <?php echo 'solid ' . $border_width . 'px ' . $border_color; ?>;  
		border-bottom: <?php echo 'solid '  . $border_width . 'px ' . $border_color; ?>;  
	}
	...
	table.invoice-table thead tr th { 
		text-align:left; 
		border: <?php echo 'solid ' . $border_width . 'px ' . $border_color; ?>;  
	}
	...
	table.invoice-table tr td.totals, table.invoice-table th.totals { 
		text-align:left; border: <?php echo 'solid ' . $border_width .'px ' . $border_color; ?>; 
	}
	...
	table.after-order-table tr td > * + table.qr-code {
	    margin-top: 1em;
	}
	...
	.after-order-table table thead{ display: table !important; }
	
	<?php } ?>

	<?php if ( 'on' === get_option( 'wp_wc_invoice_pdf_background_image_only_first_page', 'off' ) ) {

		$page_margin_top 	= self::convert_to_css_numeric( get_option( 'wp_wc_invoice_pdf_body_margin_top' ) );
		$page_margin_left	= self::convert_to_css_numeric( get_option( 'wp_wc_invoice_pdf_body_margin_left' ) );
		$header_height 		= self::convert_to_css_numeric( get_option( 'wp_wc_invoice_pdf_header_height' ) );

		$background_first_page_top = -1 * ( $page_margin_top + $header_height );
		$background_first_page_left = -1 * $page_margin_left;

		$width_image_container	= ( get_option( 'wp_wc_invoice_pdf_paper_size', 'A4' ) == 'A4' ) ? '21cm' : '8.5in';
		$height_image_container	= ( get_option( 'wp_wc_invoice_pdf_paper_size', 'A4' ) == 'A4' ) ? '29.7cm' : '11in';

	?>	
		.background-image-container {
	        position: absolute; 
	      	width: <?php echo $width_image_container; ?>; 
	      	height: <?php echo $height_image_container; ?>;
	        z-index: -1;
	        top: 0; 
	        left: 0;
	        overflow: hidden;
		...
		.background-div-image{ position: absolute !important; }  
	<?php } ?> 
</style>
