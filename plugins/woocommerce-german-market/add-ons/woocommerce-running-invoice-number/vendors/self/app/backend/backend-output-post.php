			...
			// output if it is a new order
				...
				if ( apply_filters( 'german_market_invoice_number_edit_number_in_order', true, $order ) ) { 
	                ...
	                <p class="form-field form-field-wide">
	                    ...
	                    <label for="order_generate_invoice"><?php echo esc_attr( __( 'Generate invoice number and invoice date when saving the order', 'woocommerce-german-market' ) ); ?></label>
	                </p>
	                <?php
	            }
			}
			...
			if ( isset( $_REQUEST[ 'order_generate_invoice' ] ) ) {
				...
				if ( isset( $_REQUEST[ 'order_invoice_number' ] ) ) {
					...
					if ( ! empty( trim( $_REQUEST[ 'order_invoice_number' ] ) ) ) {
						...
						$current_number = $order->get_meta( '_wp_wc_running_invoice_number' );
						$order->update_meta_data( '_wp_wc_running_invoice_number', esc_attr( $_REQUEST[ 'order_invoice_number' ] ) );
						$save = true;
						$logging = apply_filters( 'german_market_invoice_number_logging', false );

						if ( $logging ) {
							if ( $current_number != $_REQUEST[ 'order_invoice_number' ] ) {
								$new_log = sprintf( 'Order: %s, Manual Saved Invoice Number Meta from %s to %s.', $order->get_id(), $current_number, esc_attr( $_REQUEST[ 'order_invoice_number' ] ) );
								$logger = wc_get_logger();
								$context = array( 'source' => 'german-market-invoice-number' );
								$logger->info( $new_log, $context );
							}
				...
				if ( isset( $_REQUEST[ 'order_invoice_date' ] ) ) {
					...
					if ( empty( $_REQUEST[ 'order_invoice_date' ] ) ) {
						...
						$current_date 			= $order->get_meta( '_wp_wc_running_invoice_number_date' );
						...
						if ( $update_invoice_date ) {
							$order->update_meta_data( '_wp_wc_running_invoice_number_date', strtotime( $_REQUEST[ 'order_invoice_date' ] . ' ' . current_time( 'H:i' ) ) );	
							$save = true;
						}
					}
				}
