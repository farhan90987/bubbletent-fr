...
class WGM_General_Tax_Output {
	...
	*/
	private function __construct() {
		...
		if ( 'on' === $general_tax_output_activation ) {
			...
			add_action( 'wp', array( $this, 'activate_general_taxoutput_on_cart' ) );
 			...
 			if ( $this->is_cart_block_default() ) {
 				$this->cart_block_taxoutput();
 			}
	...
	/**
	 * Checks if cart block is used
	 * 
	 * @return bool
	 */
	public function is_cart_block_default() {
		$is_cart_block_default = false;

		if ( class_exists( 'Automattic\WooCommerce\Blocks\Utils\CartCheckoutUtils' ) ) {
			$is_cart_block_default = CartCheckoutUtils::is_cart_block_default();
		}

		return $is_cart_block_default;
	}
	...
	public function cart_block_taxoutput() {
		if ( $this->is_store_api_request_cart() ) {
			$this->activate_general_tax_output_totals();
			$this->avoid_same_tax_line_in_split_tax_html();
		}
	}
 	...
 	/**
 	 * Checks if we have a store api request in cart
 	 * 
 	 * @return bool
 	 */
 	public function is_store_api_request_cart() {
 		
 		$is_store_api_request_cart = false;
 		
 		if ( WC()->is_store_api_request() ) {
			if (  isset( $_SERVER[ 'HTTP_REFERER' ] ) && false !== strpos( $_SERVER[ 'HTTP_REFERER' ], wc_get_cart_url() ) ) {
				$is_store_api_request_cart = true;
			}
		}

		return $is_store_api_request_cart;
 	}
	...
	*/
	public static function product_summary_parts_after( $output_parts, $product, $hook ) {
		...
		if ( isset( $output_parts[ 'tax' ] ) && ( ! empty( $output_parts[ 'tax' ] ) ) && apply_filters( 'german_market_general_tax_output_product', true, $product ) ) {
	...
	*/
	public static function get_tax_line( $tax_line, $product ) {
		...
		if ( ( ! empty( $tax_line ) ) && apply_filters( 'german_market_general_tax_output_product', true, $product ) ) {
