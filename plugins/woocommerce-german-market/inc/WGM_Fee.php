...
class WGM_Fee {
	...
	public static function add_fee_to_gateway_page( WC_Cart $cart ) {
		...
		if ( WC()->payment_gateways ) {
			...
			$block_chosen = WC()->session->get( 'german_market_wc_blocks_active_payment_method' );
			if ( ! empty( $block_chosen ) ) {
				$chosen = $block_chosen;
			}
	...
	/**
	 * Remove session variable that is only used for checkout block
	 * 
	 * @wp-hook woocommerce_before_checkout_form
	 * @param WC_Checkout $checkout
	 */
	public static function remove_block_session( $checkout ) {
		WC()->session->set( 'german_market_wc_blocks_active_payment_method', '' );
	}
		...
		foreach( $order->get_fees() as $key => $fee ) {
			...
			if ( isset( $taxes[ 'total' ] ) ) {
				...
				foreach ( $taxes[ 'total' ] as $tax_rate_key => $tax_infos ) {
					...
					$rate[ 'sum' ]   	= $tax_infos;
					$rate[ 'rate' ]  	= WGM_Tax::get_rate_percent_by_rate_id_and_order( $tax_rate_key, $order );
					$rate[ 'rate_id' ]	= $tax_rate_key;
					$rates[ 'rates' ][] = $rate;
				}
			...
			$items[ $search_key ][ 'value' ] .= WGM_Tax::get_split_tax_html( $rates, get_option( 'woocommerce_tax_display_cart' ) );
