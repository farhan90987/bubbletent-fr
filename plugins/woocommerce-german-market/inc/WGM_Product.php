...
class WGM_Product {
	...
	public static function init() {
		...
		if ( get_option( 'gm_gtin_activation', 'off' ) == 'on' ) {
			...
			add_filter( 'woocommerce_structured_data_product',								array( 'WGM_Product', 'structured_data' ), 10, 2 );
		}

		if ( 'on' === get_option( 'german_market_variable_price_range_from', 'off' ) ) {
			add_filter( 'woocommerce_format_price_range', array( 'WGM_Product', 'from_price_instead_of_price_range' ), 10, 3 );
		}

		WGM_Product_GPSR::init();
		
		if ( 'on' === get_option( 'german_market_charging_device_activation', 'off' ) ) {
			WGM_Product_Charging_Device::get_instance();
		}
	}

	/**
	 * Change variable product price range to "from" price
	 * 
	 * @since 3.37
	 * @wp-hook woocommerce_format_price_range
	 * @param String $price
	 * @param Float $from
	 * @param Float $to
	 * @return String
	 */
	public static function from_price_instead_of_price_range( $price, $from, $to ) {
		
		$prefix = get_option( 'german_market_variable_price_range_from_prefix', _x( 'From', 'From price prefix for variable products', 'woocommerce-german-market' ) );

		/* translators: 1: prefix (word "from") 2: formatted price */
		$from_price_markup = _x( '%1$s %2$s', 'From price output', 'woocommerce-german-market' );

		$span_class = apply_filters( 'german_market_price_range_from_prefix', 'german-market-from-price-prefix' );

		return sprintf( $from_price_markup, '<span class="' . esc_attr( $span_class ) . '">' . $prefix . '</span>', is_numeric( $from ) ? wc_price( $from ) : $from );
	}
	...
	public static function add_gtin_frontend( $output_parts, $product, $hook ) {
		...
		$show = false;
		if ( 
			( 'loop' === $hook && 'on' === get_option( 'gm_gtin_loop', 'off' ) ) ||
			( 'single' === $hook && 'on' === get_option( 'gm_gtin_product_pages', 'off' ) )
		) {
			$show = true;
		}

		$show = apply_filters( 'german_market_add_gtin_frontend_show_in_hook', $show, $hook );
		...
		if ( $show ) {
			...
			$gtin = esc_attr( $product->get_meta( '_gm_gtin' ) );
	...
	public static function add_gtin_options( $loop = NULL, $variation_data = NULL, $variation = NULL ) {
		...
		if ( $is_variation ) { ?>
			...
			<div class="german-market-gtin" style="border: 1px solid #eee; padding: 10px; box-sizing: border-box; display: inline-block; width: 100%;">
		...
		<?php } ?>
			...
			<p class="<?php echo $class; ?>" style="display:block">
				...
				<input type="text" name="_gm_gtin<?php echo $name_suffix; ?>" id="_gm_gtin<?php echo $name_suffix; ?>" value="<?php echo esc_attr( $data ); ?>" <?php echo $input_style; ?>/>
	...
	public static function structured_data( $markup, $product ) {
		...
		if ( ! empty( $key ) ) {
			if ( ! isset( $markup[ $key ] ) ) {
				$markup[ $key ] = apply_filters( 'german_market_wgm_product_gtin_structured_data', (string)$gtin, $markup, $product );
			}
		}
	...
	public static function add_cart_item_data( $cart_item_data, $product_id, $variation_id, $quantity = 1 ) {
		...
		if ( ( ! is_null( $product ) ) && WGM_Helper::method_exists( $product, 'get_attributes' ) ) {
			...
			foreach ( $attributes as $attribute ) {
				...
				if ( str_replace( 'variable', '', $product->get_type() ) === $product->get_type() ) {
					$is_variation = false;
				}

				if ( $attribute->get_visible() && ( ! $is_variation ) ) {
					...
					if ( is_array( $option_names ) && ( count( $option_names ) > 1 ) && apply_filters( 'german_market_attribute_name_add_to_cart_do_asort', '__return_true' ) ) {
						asort( $option_names );
					}
	...
	public static function init_product_images() {
		...
		$show_images_in = array(
			...
			'email'		=> get_option( 'german_market_product_images_in_emails', 'standard' ),
		);
		...
		if ( 'on' === $show_images_in[ 'email' ] || 'off' === $show_images_in[ 'email' ] ) {
	...
	public static function add_thumbnail_to_emails( $args ) {
		...
		$args[ 'show_image' ] = 'on' === get_option( 'german_market_product_images_in_emails', 'standard' );
		return $args;
	}
