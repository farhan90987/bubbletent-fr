...
abstract class MarketPress_Improve_Plugin {
	...
	final public function __construct() {
		...
		/**
		 * Filter to disable plugin improver
		 * 
		 * @since 1.0
		 * @param Boolean
		 */
		...
		if ( 1 == self::$instance_counter ) {
			...
			load_textdomain( 'marketpress-plugin-improve', untrailingslashit( Woocommerce_German_Market::$plugin_path ) . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'marketpress-improve-plugin' . DIRECTORY_SEPARATOR . 'languages' . DIRECTORY_SEPARATOR . 'marketpress-plugin-improve-' . $locale . '.mo' );
	...
	private function have_to_run() {
		...
		if ( ! isset( $marketpress_improve_option[ $this->plugin_slug ] ) ) {
			...
			if ( 'done' === $marketpress_improve_option[ $this->plugin_slug ] ) {
		...
		if ( isset( $_REQUEST[ 'marketpress-agree-' . $this->plugin_slug ] ) || isset( $_REQUEST[ 'marketpress-disagree-' . $this->plugin_slug ] ) ) {
			...
			if ( isset( $_REQUEST[ 'marketpress-agree-' . $this->plugin_slug ] ) ) {
				...
				<div class="notice notice-success is-dismissible">
					<p><?php echo wp_kses_post( $this->get_notice_success_text() ); ?></p>
				</div>
				<?php
			}
		...
		<div class="notice notice-warning">
			...
			<table style="border: 1px solid #ddd; padding: 20px; display:none;">
				...
				foreach ( $data[ 'global_data' ] as $key => $value ) {
					?>
					<tr>
						<td style="padding-right: 10px;"><?php echo esc_attr( $key ); ?>:</td>
				...
				?>
					<tr>
						...
						<td style="padding-top: 10px;"><?php echo esc_attr( $this->plugin_slug ); ?></td>
					</tr>

					<tr>
						<td style="padding-bottom: 10px;padding-right: 10px;">Plugin Version:</td>
						<td  style="padding-bottom: 10px;"><?php echo esc_attr( $this->plugin_version ); ?></td>
					</tr>
			...
			<p>
				<form method="post" name="marketpress-improve-<?php echo esc_attr( $this->plugin_slug ); ?>">
					<input type="submit" class="button-secondary" value="<?php echo esc_attr( $this->get_agree_button_text() ); ?>" name="marketpress-agree-<?php echo esc_attr( $this->plugin_slug ); ?>"/>
					<input type="submit" class="button-secondary" value="<?php echo esc_attr( $this->get_disagree_button_text() ); ?>" name="marketpress-disagree-<?php echo esc_attr( $this->plugin_slug ); ?>"/>
				</form>
			</p>
