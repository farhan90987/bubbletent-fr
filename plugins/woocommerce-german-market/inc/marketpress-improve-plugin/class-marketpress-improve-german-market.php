...
class MarketPress_Improve_GermanMarket extends MarketPress_Improve_Plugin {
	...
	final protected function get_plugin_data() {
		...
		foreach ( $add_on_files as $add_on_id => $add_on ) {
			...
			if ( 'woocommerce-shipping' === strtolower( $add_on_id ) ) {
				// Check for available WooCommerce Shipping Providers
				$providers = array(
					'dhl'
				);
				if ( count( $providers ) > 0 ) {
					foreach( $providers as $provider ) {
						$option_key = 'wgm_add_on_' . str_replace( '-', '_', strtolower( $add_on_id ) ) . '_' . strtolower( $provider );
						$data[ 'Add-On ' . $add_on_id . ' (' . strtoupper( $provider ) . ')' ] = get_option( $option_key, 'off' );
					}
				}
			} else {
		...
		// Options
		$data[ 'Option Customizable Product Data' ]				= get_option( 'german_market_customizable_product_data_customized', 'off' );
		...
		$data[ 'Option charging device' ]						= get_option( 'german_market_charging_device_activation', 'off' );
		...
		$data[ 'HPOS' ] = WGM_Hpos::is_hpos_enabled() ? 'on' : 'off';

		// Lexware Office - Is "outbounding documents" used interface
		if ( 'on' === get_option( 'wgm_add_on_lexoffice', 'off' ) ) {

			$is_lexware_invoice_api = 'invoice' === get_option( 'woocommerce_de_lexoffice_voucher_or_invoice', 'voucher' );
			$data[ 'Lexware Outbounding API' ] = $is_lexware_invoice_api ? 'on' : 'off';
		}

		// ZUGFeRD Invoices
		if ( 'on' === get_option( 'wgm_add_on_woocommerce_invoice_pdf', 'off' ) ) {
			$data[ 'ZUGFeRD-Invoices' ] = get_option( 'german_market_einvoice_send_zugferd_invoices', 'off' );
		}
