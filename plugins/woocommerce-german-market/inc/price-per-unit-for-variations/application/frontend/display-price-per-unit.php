...
add_filter( 'wgm_product_summary_parts', 'wcppufv_add_price_per_unit', 10, 3 );
 ...
 * @return Array
 */
function wcppufv_add_price_per_unit( $parts = array(), $product = null, $hook = 'single', $option_check = true ) {

	if ( $option_check ) {	
		$option_key = 'woocommerce_de_show_price_per_unit_' . $hook;
		if ( 'off' === get_option( $option_key, 'on' ) ) {
			return $parts;
		}
	}
...
function wcppufv_get_price_per_unit_string_by_variable_product( $product ) {
	...
	$return_string .= apply_filters(
		...
		$price_per_unit,
		$price_per_unit_mult,
		apply_filters( 'german_market_measuring_unit', $price_per_unit_unit, $product )
	);
...
function wcppufv_get_price_per_unit_string_by_product( $product ) {
	...
	if ( isset( $price_per_unit_data[ 'price_per_unit' ] ) && $price_per_unit_data[ 'price_per_unit' ] ) {
