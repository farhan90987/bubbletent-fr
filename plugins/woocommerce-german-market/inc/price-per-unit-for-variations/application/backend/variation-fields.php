	...
	<tr>
		<td colspan="2" >
			...
			<div class="german-market-ppu-variation" style="border: 1px solid #eee; padding: 10px; box-sizing: border-box;">
				...
				<b><?php esc_attr_e( 'Price Per Unit', 'woocommerce-german-market' ); ?>:</b>
...
function wcppufv_add_product_write_panel_automatic_calculation( $loop, $variation_data, $variation ) {
	...
	<p class="form-field _regular_price_per_unit_field ppu_variable ppu_auto_calc">
		<label <?php esc_attr_e( $label_style ); ?> for="variable_used_setting_ppu[<?php esc_attr_e( $loop ); ?>]"><?php esc_attr_e( 'Used Setting', 'woocommerce-german-market' ); ?>:</label>

		<select name="variable_used_setting_ppu[<?php esc_attr_e( $loop ); ?>]" class="variable_used_setting_ppu" data-loop="<?php esc_attr_e( $loop ); ?>">
			<option value="-1"><?php esc_attr_e( 'Same as parent', 'woocommerce-german-market' ); ?></option>
			<option value="1" <?php esc_attr_e( $special_is_set ); ?>><?php esc_attr_e( 'Following Special Variation Setting', 'woocommerce-german-market' ); ?></option>
		</select>

	</p>

	<div id="gm_ppu_auot_calc_parent_special[<?php esc_attr_e( $loop );?>]" class="gm_ppu_auot_calc_parent_special gm_ppu_auot_calc_parent_special_<?php esc_attr_e( $loop ); ?>" <?php echo wp_kses_post( $show_settings ); ?>>

		<p class="form-field _regular_price_per_unit_field ppu_variable ppu_auto_calc">
			<label <?php echo wp_kses_post( $label_style ); ?> for="_unit_regular_price_per_unit"><?php esc_attr_e( 'Scale Unit', 'woocommerce-german-market' ); ?>:</label>
			<?php echo wcppufv_select_scale_units( $variation->ID, $regular_price_per_unit_selection ); ?>
		</p>

		<p class="form-field _regular_price_per_unit_field ppu_variable ppu_auto_calc">
			<label <?php echo wp_kses_post( $label_style ); ?> for="variable_auto_ppu_complete_product_quantity[<?php esc_attr_e( $loop ); ?>]"><?php esc_attr_e( 'Complete product quantity', 'woocommerce-german-market' ); ?>:</label>
			<input <?php echo wp_kses_post( $input_style ); ?> type="number" min="0" step="<?php esc_attr_e( apply_filters( 'german_market_auto_ppu_step', '0.0001' ) ); ?>" name="variable_auto_ppu_complete_product_quantity[<?php esc_attr_e( $loop ); ?>]" id="variable_auto_ppu_complete_product_quantity[<?php esc_attr_e( $loop ); ?>]" value="<?php esc_attr_e( get_post_meta( $variation->ID, '_v_auto_ppu_complete_product_quantity', TRUE ) ); ?>" class="german-market-variation-input-not-translatable"/>
		</p>

		<p class="form-field _regular_price_per_unit_field ppu_variable ppu_auto_calc" style="clear: both;">
			<label <?php echo wp_kses_post( $label_style ); ?> for="variable_unit_regular_price_per_unit_mult[<?php esc_attr_e( $loop ); ?>]"><?php esc_attr_e( 'Quantity to display', 'woocommerce-german-market' ); ?>:</label>
			<input <?php echo wp_kses_post( $input_style ); ?> type="number" min="0" step="<?php esc_attr_e( apply_filters( 'german_market_auto_ppu_step', '0.0001' ) ); ?>" name="variable_unit_regular_price_per_unit_mult[<?php echo $loop; ?>]" id="variable_unit_regular_price_per_unit_mult[<?php esc_attr_e( $loop ); ?>]" value="<?php esc_attr_e( get_post_meta( $variation->ID, '_v_unit_regular_price_per_unit_mult', TRUE ) ); ?>" class="german-market-variation-input-not-translatable"/>
		</p>
