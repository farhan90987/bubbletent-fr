...
class WGM_Legal_Information_Product_Reviews {
	...
	*/
	private function __construct() {
		...
		if ( 'nothing' !== $this->star_ratings_option ) {
			...
			add_filter( 'woocommerce_product_get_rating_html', array( $this, 'rating_html' ), 11, 3 );
	...
	*/
	public function review_before() {
		remove_filter( 'woocommerce_product_get_rating_html', array( $this, 'rating_html' ), 11, 3 );
	}
	...
	*/
	...
	/**
	* Return Markup For Short Text Information
	* 
	* @return String
	*/
	public function get_short_text_markup() {
		
		$unique_id = uniqid( 'german-market-legal-information-full-text-' );
		$short_text = get_option( 'gm_legal_information_product_reviews_short_information_text', self::get_short_text_default() );
		$close_element = apply_filters( 'german_market_legal_info_product_reviews_short_text_close_element','<span class="close-full-text" role="button" tabindex="0" aria-controls="' . $unique_id . '">' . __( 'Close', 'woocommerce-german-market' ) .'</span>' );
		$info_icon = apply_filters( 'german_market_legal_info_product_reviews_short_text_info_icon', '<span class="german-market-legal-information-for-product-reviews-info-icon" aria-hidden="true">ⓘ</span>' );
		
		$markup = apply_filters( 'german_market_legal_info_product_reviews_short_text_markup',

					'<span class="german-market-legal-information-for-product-reviews short-after-star-rating" role="button" tabindex="0" aria-controls="' . $unique_id . '" aria-haspopup="true">' .
						$info_icon . 
						'%s<span class="full-text" aria-hidden="true" aria-expanded="false" id="' . $unique_id . '">' . 
							$close_element . '%s
						</span>
					</span>' );
