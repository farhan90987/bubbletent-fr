...
class WGM_Plugin_Compatibility_PayPal_Payments {
	...
	private function __construct() {
		...
		add_filter( 'german_market_double_opt_do_not_logout_logged_in_not_checkout_waiting', array( $this, 'double_opt_in_do_not_logout' ) );
	...
	/**
	 * Avoid redirct to my account after payment
	 *
	 * 
	 * @since 3.25
	 * @wp-hook german_market_double_opt_do_not_logout_logged_in_not_checkout_waiting
	 * 
	 * @param Boolean $do_not_logut
	 * @return Boolean
	 */
	public function double_opt_in_do_not_logout( $do_not_logout = false ) {

		if ( 
			isset( $_REQUEST[ 'wc-ajax' ] ) && 
			'ppc-return-url' === $_REQUEST[ 'wc-ajax' ]
		) {
			$do_not_logout = true;
		}

		return $do_not_logout;
	}
}
