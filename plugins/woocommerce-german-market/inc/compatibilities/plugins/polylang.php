...
class WGM_Plugin_Compatibility_Polylang {
	...
	*/
	private function __construct() {
		...
		add_filter( 'gm_delivery_time_value_in_checkout', array( $this, 'delivery_time_value_in_checkout' ), 10, 2 );
		...
		add_action( 'admin_init', function() {
			if ( 'on' === get_option( 'wgm_add_on_fic', 'off' ) ) {
				...
				foreach ( $terms as $term ) {
					pll_register_string( $term->name, $term->name, 'German Market: Nutritional Values', true );
				}
			}
		});
		
		add_filter( 'gm_fic_nutritional_values_term_name', function( $term_name, $term, $is_backend = false ) {

			if ( ! $is_backend ) {
				$term_name = pll__( $term_name );
			}
			return $term_name;
		}, 10, 3 );
		...
		if ( get_option( 'wgm_add_on_woocommerce_invoice_pdf', 'off' ) == 'on' ) {
			...
			add_filter( 'wp_wc_invoice_pdf_additional_pdf_tac_pages_array',			array( $this, 'polylang_additional_pdf_pages' ) );
			add_filter( 'wp_wc_invoice_pdf_additional_pdf_ecovation_pages_array',	array( $this, 'polylang_additional_pdf_pages' ) );
		}
	...
	/**
	* Translate delivery times in cart and checkout
	*
	* @since v3.16
	...
	* @return String
	*/
	function delivery_time_value_in_checkout ( $delivery_time, $cart_item ) {
	...
	*/
	...
	/**
	* Translate Pages of Addional PDFs in Invoice PDF Add-On
	*
	* @since v3.32
	...
	* @return Array
	*/
	public function polylang_additional_pdf_pages( $pages ) {

		foreach ( $pages as $key => $page ) {
			$pages[ $key ] = pll_get_post( $page->ID, pll_current_language() );
		}
