...
class WGM_Plugin_Compatibility_WPML {
	...
	*/
	private function __construct() {
		...
		if ( function_exists( 'icl_register_string' ) && function_exists( 'icl_t' ) && function_exists( 'icl_st_is_registered_string' ) && function_exists( 'wcml_loader' ) ) {
			...
			add_filter( 'german_market_wpml_translate_page_id',									array( $this, 'translate_page_id' ) );
		...
		if ( is_admin() && class_exists( 'WCML_Admin_Menus' ) && class_exists( 'SitePress' ) && ( ( get_option( 'wgm_add_on_woocommerce_invoice_pdf', 'off' ) == 'on' ) || ( get_option( 'wgm_add_on_woocommerce_return_delivery_pdf', 'off' ) == 'on' ) ) ) {
			...
			add_action( 'german_market_before_bulk_for_pdfs', function() {
				do_action( 'wpml_st_force_translate_admin_options', array_keys( WGM_Helper::get_translatable_options() ) );
			});
	...
	*/
	public function wpml_translate_delivery_times( $label_term, $product ) {
		...
		if ( $original_product_id > 0 ) {
			...
			if ( empty( $term_id ) || ( -1 === intval( $term_id  ) ) || ( 0 === intval( $term_id ) ) ) {
	...
	*/
	...
	/**
	 * Get page id of translated page in current language
	 * 
	 * @wp-hook german_market_wpml_translate_page_id
	 * @param Integer $page_id
	 * @return Integer
	 */
	public function translate_page_id( $page_id ) {
		return icl_object_id( $page_id );
	}
				...
				if ( WGM_Helper::method_exists( $order, 'get_meta' ) ) {
					...
					if ( ! empty( $order_language ) ) {
						...
						if ( isset( $all_gateways[ $payment_method_id ] ) ) {
							...
							$settings_language_title = false;

							if ( isset( $settings[ 'title' ] ) ) {
								$settings_language_title = $settings[ 'title' ];
							} else if ( isset( $used_gateway->title ) && ( ! empty( $used_gateway->title ) ) )  {
								$settings_language_title = $used_gateway->title;
							} else if ( isset( $used_gateway->method_title ) && ( ! empty( $used_gateway->method_title ) ) ) {
								$settings_language_title = $used_gateway->method_title;
							}
	...
	*/
	public function wpml_js_lock_fields_ids( $ids ) {
		...
		$ids[] = '_german_market_gpsr_ignore_defaults';
		$ids[] = '_german_market_gpsr_manufacturer';
		$ids[] = '_german_market_gpsr_responsible_person';
		$ids[] = '_german_market_charging_device_pictogram';
		$ids[] = '_german_market_charging_device_minimum_power';
		$ids[] = '_german_market_charging_device_maximum_power';
		$ids[] = '_german_market_charging_device_usb_pd';
		$ids[] = '_german_market_charging_device_variation_setting';
		return $ids;
	}
