...
class WGM_Due_date {
	...
	*/
	private function __construct() {
		...
		if ( 'on' === get_option( 'woocommerce_de_due_date', 'off' ) ) {
			...
			add_filter( 'german_market_gateway_settings_single_gateway', array( $this, 'settings_field' ), 11, 2 );
			add_action( 'woocommerce_checkout_update_order_meta', array( $this, 'save_due_date_in_order' ), 20, 2 );
			add_action( 'woocommerce_store_api_checkout_update_order_from_request', array( $this, 'save_due_date_in_order_store_api' ) );
		...
		} else if ( 'on' === get_option( 'wgm_add_on_lexoffice', 'off' ) && 'invoice' === get_option( 'woocommerce_de_lexoffice_voucher_or_invoice', 'voucher' ) ) {
	...
	public function save_due_date_in_order_store_api( $order ) {
		$this->save_due_date_in_order( 0, true, false, $order );
	}
	...
	* @return void
	**/
	public function save_due_date_in_order( $order_id, $checkout = false, $payment_method_id = false, $order = null ) {
		...
		$order->update_meta_data( '_wgm_due_date', $due_date_string );
	...
	*/
	public function settings_field( $settings, $gateway_id ) {
		...
		if ( ! isset( $settings[ 'lexoffice_due_date' ] ) ) {
			...
			// due date file
			$file = dirname( Woocommerce_German_Market::$plugin_filename ) . DIRECTORY_SEPARATOR . 'add-ons' . DIRECTORY_SEPARATOR . 'lexoffice' .DIRECTORY_SEPARATOR . 'application' . DIRECTORY_SEPARATOR . 'due-date.php';
			if ( file_exists( $file ) ) {
				require_once( $file );
				$settings = lexoffice_woocommerce_due_date_settings_field( $settings, $gateway_id );
			}
		}
		...
		if ( function_exists( 'lexoffice_woocommerce_init' ) ) {
			$settings[ 'lexoffice_due_date_title' ][ 'description' ] .= ' ' . __( 'It is also used for Lexware Office vouchers.', 'woocommerce-german-market' );
		}

		$settings[ 'lexoffice_due_date' ][ 'title' ] = __( 'Due Date', 'woocommerce-german-market' );

		// Text
		$current_payment_gateway = $gateway_id;
		...
		$settings[ 'wgm_due_date_text' ] = array(
			...
			'description'		=> __( 'You can use the following placeholders: Due Date - <code>{{due-date}}</code>, Days - <code>{{days}}</code>.', 'woocommerce-german-market' ),
		...
		if ( $due_date != '' ) {
			...
			if ( $payment_method_id != '' ) {
				...
				if ( isset( $gateways[ $payment_method_id ] ) ) {
					...
					$gateway_setting_due_date = WGM_Payment_Settings::get_option( 'lexoffice_due_date', $gateway );
					if ( ! empty( $gateway_setting_due_date) ) {
						$due_date_days 	= intval( $gateway_setting_due_date );
					} else {
	...
	public function save_meta_data( $order_id, $post_or_order_object ) {
		...
		if ( isset( $_REQUEST[ '_wgm_due_date' ] ) ) {
			...
			if ( $_REQUEST[ '_wgm_due_date' ] != '' ) {
				...
				$order->update_meta_data( '_wgm_due_date', $_REQUEST[ '_wgm_due_date' ] );
				$order->save_meta_data();
				
			} else {
				$this->save_due_date_in_order( $order_id, null );
			}
