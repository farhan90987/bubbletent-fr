...
class WGM_Manual_Order_Confirmation {
	...
	*/
	...
	public function order_needs_payment_checkout_block( $needs_payment, $order, $valid_order_statuses ) {
		...
		} else if ( 'yes' === $order->get_meta( '_gm_needs_conirmation' ) ) {
			$needs_payment = false;
		}
	...
	public function set_payment_complete_order_status_checkout_block ( $status, $order_id, $order ) {
		...
		if ( 'yes' === $order->get_meta( '_gm_needs_conirmation' ) ) {
			$status = 'pending';
			if ( 'on' === get_option( 'woocommerce_de_manual_order_confirmation_payment_methods_only_pay_order', 'on' ) ) {
	...
	*/
	public static function new_text_in_order_on_hold_mail_processing( $translated, $original, $domain ) {
		...
		$search_on_hold_3 		= 'Thanks for your order. It’s on-hold until we confirm that payment has been received.';
		...
		if ( 'woocommerce' === $domain ) {
			...
			if ( $email_improvements_enabled ) {
				...
				if ( $original === 'We’ve received your order and it’s currently on hold until we can confirm your payment has been processed.' ) {
					$translated = __( 'Your order has been confirmed.', 'woocommerce-german-market' );

				} else if ( $original === 'Here’s a reminder of what you’ve ordered:' ) {
					$translated = __( 'Your order is now being processed.', 'woocommerce-german-market' );
				}

			} else {

				if ( ( $original == $search_on_hold ) || ( $original == $search_on_hold_2 ) || ( $original == $search_on_hold_3 ) ) {
		...
		if ( 'woocommerce' === $domain ) {
			...
			if ( $email_improvements_enabled ) {
				...
				if ( $original === 'We’ve received your order and it’s currently on hold until we can confirm your payment has been processed.' ) {
					...
					$translated = __( 'Your order is on-hold until we confirm payment has been received.', 'woocommerce-german-market' );
				}
