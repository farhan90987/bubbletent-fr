...
class WGM_Compatibilities {
	 ...
	 * @var object[]|null
	 */
	private static $shipping_methods = null;
	...
	*/
	private function __construct() {
		...
		if ( class_exists( 'MarketPress_Salesman' ) && class_exists( 'MarketPress_Salesman_Free_Shipping_Notice_Application' ) ) {
			add_action( 'wgm_shipping_method_after_init', function( $shipping_method ) {

				if ( $shipping_method->is_enabled() && ( ( ( floatval( $shipping_method->cost ) == 0 ) && ( floatval( $shipping_method->minimum_amount ) > 0 ) ) || ( floatval( $shipping_method->free_min_amount ) > 0 ) ) ) {

					if ( ! is_null( self::$shipping_methods ) && array_key_exists( $shipping_method->id, self::$shipping_methods ) ) {
						return;
					}

					self::$shipping_methods[ $shipping_method->id ] = $shipping_method;

					add_action( 'marketpress_salesman_before_get_cheapest_free_shipping_method', function() {
						add_filter( 'marketpress_salesman_free_shipping_notice_free_shipping_methods', array( __CLASS__, 'salesman_add_free_shipping_method' ) );
					} );

					add_action( 'marketpress_salesman_after_get_cheapest_free_shipping_method', function() {
						remove_filter( 'marketpress_salesman_free_shipping_notice_free_shipping_methods', array( __CLASS__, 'salesman_add_free_shipping_method' ) );
					} );

					if ( method_exists( $shipping_method, 'is_free_shipping_available' ) && $shipping_method->is_free_shipping_available() ) {
						if ( class_exists( 'CSP_ShippingManager' ) && method_exists( 'CSP_ShippingManager', 'add_free_shipping_method_id' ) ) {
							CSP_ShippingManager::add_free_shipping_method_id( $shipping_method->id );
						}

						if ( class_exists( 'BM_Blocks' ) && method_exists( 'BM_Blocks', 'add_free_shipping_method_id' ) ) {
							BM_Blocks::add_free_shipping_method_id( $shipping_method->id );
						}

						if ( class_exists( 'MarketPress\GermanMarket\Shipping\Woocommerce_Shipping' ) && method_exists( 'MarketPress\GermanMarket\Shipping\Woocommerce_Shipping', 'add_free_shipping_method_id' ) ) {
							\MarketPress\GermanMarket\Shipping\Woocommerce_Shipping::add_free_shipping_method_id( $shipping_method->id );
						}

						if ( class_exists( 'WGM_Template' ) && method_exists( 'WGM_Template', 'add_free_shipping_method_id' ) ) {
							WGM_Template::add_free_shipping_method_id( $shipping_method->id );
						}
					}
				}
		...
		******************************/
		...
		/******************************
		// WPC Product Bundles & DHL Shipping Addon
		******************************/
		if ( class_exists( 'WPCleverWoosb' ) ) {
			require_once( Woocommerce_German_Market::$plugin_path . 'inc' . DIRECTORY_SEPARATOR . 'compatibilities' . DIRECTORY_SEPARATOR . 'plugins' . DIRECTORY_SEPARATOR . 'wpc-product-bundles.php' );
			$wpcpb_compatibility = WGM_Plugin_Compatibility_WPC_Product_Bundles::get_instance();
		}
		...
		******************************/
		...
		if ( class_exists( 'WC_Subscriptions' ) ) {
			...
			add_filter( 'wgm_gateway_purchase_on_account_process_payment_update_status', function( $update_status, $order ) {
			...
			// purchase on account, order status for renewals like in payment setting
			add_filter( 'wcs_renewal_order_created', function( $renewal_order, $subscription ) {
				...
				if ( is_object( $subscription ) && method_exists( $subscription, 'get_payment_method' ) ) {
					if ( 'german_market_purchase_on_account' === $subscription->get_payment_method() ) {
						$new_status = get_option( 'woocommerce_german_market_purchase_on_account_order_status', 'processing' );
		...
		******************************/
		...
		$added_theme_compatibilitiy = true;
		$the_theme = wp_get_theme();

		if ( $the_theme->get( 'TextDomain' ) == 'thb_text_domain' || ( $the_theme->get_template() == 'superba' ) ) {
			$this->add_my_theme_action( 'init', array( $this, 'theme_support_superba' ), 20 );
		}

		// Theme woodance
		else if ( ( ! is_admin() ) && ( $the_theme->get( 'TextDomain' ) == 'woodance' || ( $the_theme->get_template() == 'woodance' ) ) ) {
			$this->add_my_theme_action( 'wp', array( $this, 'theme_support_woodance' ) );
		}

		// Theme Envision
		else if ( $the_theme->get_template() == 'envision' || $the_theme->get_stylesheet() == 'envision' ) {
			$this->add_my_theme_action( 'german_market_after_frontend_init', array( $this, 'theme_support_envision' ) );
		}

		// Theme Fluent
		else if ( $the_theme->get_template() == 'fluent' || $the_theme->get_stylesheet() == 'fluent' || $the_theme->get_stylesheet() == 'fluent-child' ) {
			$this->add_my_theme_action( 'wp', array( $this, 'theme_support_fluent' ) );
		}

		// Theme Peddlar
		else if ( $the_theme->get_template() == 'peddlar' || $the_theme->get_stylesheet() == 'peddlar' || $the_theme->get_stylesheet() == 'peddlar-child' ) {
			$this->add_my_theme_filter( 'wgm_close_a_tag_before_wgm_product_summary_in_loop', '__return_false' );
			$this->add_my_theme_filter( 'wgm_product_summary_html', array( $this, 'theme_support_peddlar' ), 1, 4 );
		}
		...
		// Theme VG Vegawine
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_vegawine' ) );
		}

		// Theme Savoy
		else if ( $the_theme->get_template() == 'savoy' || $the_theme->get_stylesheet() == 'savoy' || $the_theme->get_stylesheet() == 'savoy-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_savoy' ) );
		}

		// Theme Kryia
		else if ( $the_theme->get_template() == 'kriya' || $the_theme->get_stylesheet() == 'kriya' || $the_theme->get_stylesheet() == 'kriya-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_kriya' ) );
		}
		...
		// handsome-shop
			...
			$this->add_my_theme_action( 'init', array( $this, 'theme_support_handmade_shop' ), 11 );
		...
		// iustore
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_iustore' ) );

		// amely
		} else if ( $the_theme->get_template() == 'amely' || $the_theme->get_stylesheet() == 'amely' || $the_theme->get_stylesheet() == 'amely-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_amely' ), 11 );			
		...
		// Ordo
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_ordo' ), 10 );

		// Variegated
		} else if ( $the_theme->get_template() == 'variegated' || $the_theme->get_stylesheet() == 'variegated' || $the_theme->get_stylesheet() == 'variegated-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_variegated' ), 10 );
			$this->add_my_theme_action( 'wp_head', array( $this, 'theme_support_variegated_css' ) );

		// Adorn
		} else if ( $the_theme->get_template() == 'adorn' || $the_theme->get_stylesheet() == 'adorn' || $the_theme->get_stylesheet() == 'adorn-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_adorn' ), 10 );
		...
		// Hypermarket
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_hypermarket' ), 10 );
		...
		// Planetshine Polaris
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_planetshine_polaris' ), 10 );

		// Electro
		} else if ( $the_theme->get_template() == 'electro' || $the_theme->get_stylesheet() == 'electro' || $the_theme->get_stylesheet() == 'electro-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_electro' ), 11 );
			$this->add_my_theme_filter( 'german_market_compatibility_elementor_price_data', '__return_false' );


		// Justshop
		} else if ( $the_theme->get_template() == 'justshop' || $the_theme->get_stylesheet() == 'justshop' || $the_theme->get_stylesheet() == 'justshop-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_jutshop' ), 11 );
		...
		// Page Builder Framework
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_page_builder_framework' ), 11 );
		...
		// DFD Native
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_dfd_native' ), 11 );

		// Depot
		} else if ( $the_theme->get_template() == 'depot' || $the_theme->get_stylesheet() == 'depot' || $the_theme->get_stylesheet() == 'depot-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_depot' ), 11 );

		// Minera
		} else if ( $the_theme->get_template() == 'minera' || $the_theme->get_stylesheet() == 'minera' || $the_theme->get_stylesheet() == 'minera-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_minera' ), 11 );

		// Elaine
		} else if ( $the_theme->get_template() == 'elaine' || $the_theme->get_stylesheet() == 'elaine' || $the_theme->get_stylesheet() == 'elaine-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_elaine' ), 11 );
			$this->add_my_theme_action( 'wp_head', array( $this, 'theme_support_elaine_css' ) );
		...
		// Yolo Robino
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_yolo_robino' ), 11 );

		// Appetito
		} else if ( $the_theme->get_template() == 'appetito' || $the_theme->get_stylesheet() == 'appetito' || $the_theme->get_stylesheet() == 'appetito-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_appetito' ), 11 );

		// Superfood
		} else if ( $the_theme->get_template() == 'superfood' || $the_theme->get_stylesheet() == 'superfood' || $the_theme->get_stylesheet() == 'superfood-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_superfood' ), 11 );

		// TM Robin
		} else if ( $the_theme->get_template() == 'tm-robin' || $the_theme->get_stylesheet() == 'tm-robin' || $the_theme->get_stylesheet() == 'tm-robin-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_tm_robin' ), 11 );
			$this->add_my_theme_action( 'wp_head', array( $this, 'theme_support_tm_robin_css' ) );

		// Grosso
		} else if ( $the_theme->get_template() == 'grosso' || $the_theme->get_stylesheet() == 'grosso' || $the_theme->get_stylesheet() == 'grosso-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_grosso' ), 11 );
			$this->add_my_theme_action( 'wp_head', array( $this, 'theme_support_grosso_css' ) );

		// Uncode
		} else if ( $the_theme->get_template() == 'uncode' || $the_theme->get_stylesheet() == 'uncode' || $the_theme->get_stylesheet() == 'uncode-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_support_uncode' ), 11 );
		...
		// Calafate
			...
			$this->add_my_theme_filter( 'german_market_jquery_no_conflict', function( $rtn ) { return 'no'; } );
		...
		// Theme Highlight
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_highlight' ) );

		// Theme Oxygen
		} else if ( $the_theme->get_template() == 'oxygen' || $the_theme->get_stylesheet() == 'oxygen' || $the_theme->get_stylesheet() == 'oxygen-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_oxygen' ) );

		// Mesmerize
		} else if ( $the_theme->get_template() == 'mesmerize' || $the_theme->get_stylesheet() == 'mesmerize' || $the_theme->get_stylesheet() == 'mesmerize-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_mesmerize' ) );
		...
		// Shoptimizer Version: 1.7.1
			...
			$this->add_my_theme_filter( 'german_market_compatibility_elementor_price_data', '__return_false' ); // Elementor

		// Kameleon
		} else if ( $the_theme->get_template() == 'kameleon' || $the_theme->get_stylesheet() == 'kameleon' || $the_theme->get_stylesheet() == 'kameleon-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_kameleon' ) );

		// Makali
		} else if ( $the_theme->get_template() == 'makali' || $the_theme->get_stylesheet() == 'makali' || $the_theme->get_stylesheet() == 'makali-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_makali' ) );
		...
		// DIVI
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_divi' ) );
			$this->add_my_theme_filter( 'german_market_email_footer_the_content_filter', '__return_false' );

		// NaturaLife
		} else if ( $the_theme->get_template() == 'naturalife' || $the_theme->get_stylesheet() == 'naturalife' || $the_theme->get_stylesheet() == 'naturalife-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_naturalife' ) );

		// Werkstatt
		} else if ( $the_theme->get_template() == 'werkstatt' || $the_theme->get_stylesheet() == 'werkstatt' || $the_theme->get_stylesheet() == 'werkstatt-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_werkstatt' ) );

		// Coi
		} else if ( $the_theme->get_template() == 'coi' || $the_theme->get_stylesheet() == 'coi' || $the_theme->get_stylesheet() == 'coi-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_coi' ) );

		// Vermeil
		} else if ( $the_theme->get_template() == 'vermeil' || $the_theme->get_stylesheet() == 'vermeil' || $the_theme->get_stylesheet() == 'vermeil-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_vermeil' ) );

		// Toyshop (Storefront Child)
		} else if ( $the_theme->get_stylesheet() == 'toyshop' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_toyshop' ) );
		...
		// Kava
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_kava' ) );

		// Bolge
		} else if ( $the_theme->get_template() == 'bolge' || $the_theme->get_stylesheet() == 'bolge' || $the_theme->get_stylesheet() == 'bolge-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_bolge' ) );

		// Eveland
		} else if ( $the_theme->get_template() == 'eveland' || $the_theme->get_stylesheet() == 'eveland' || $the_theme->get_stylesheet() == 'eveland-child' ) {
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'theme_eveland' ) );
		...
		// Massive Dynamic
			...
			$this->add_my_theme_action( 'german_market_after_init', array( $this, 'massive_dynamic' ) );

		// Stockholm
		} else if ( $the_theme->get_template() == 'stockholm' || $the_theme->get_stylesheet() == 'stockholm' || $the_theme->get_stylesheet() == 'stockholm-child' ) {
			$this->add_my_theme_action( 'german_market_after_init',  array( $this, 'theme_stockholm' ), 20 );;

		// Beaver Builder Theme
		} else if ( $the_theme->get_template() == 'bb-theme' || $the_theme->get_stylesheet() == 'bb-theme' || $the_theme->get_stylesheet() == 'bb-theme-child' ) {
			$this->add_my_theme_action( 'german_market_after_init',  array( $this, 'theme_bb_theme' ), 20 );

		// Open Shop
		} else if ( $the_theme->get_template() == 'open-shop' || $the_theme->get_stylesheet() == 'open-shop' || $the_theme->get_stylesheet() == 'open-shop-child' ) {
			$this->add_my_theme_action( 'german_market_after_init',  array( $this, 'theme_open_shop' ), 20 );
		
		// Neve
		} else if ( $the_theme->get_template() == 'neve' || $the_theme->get_stylesheet() == 'neve' || $the_theme->get_stylesheet() == 'neve-child' ) {
			$this->add_my_theme_action( 'german_market_after_init',  array( $this, 'theme_neve' ), 50 );
		
		} else {
			$added_theme_compatibilitiy = false;
		}

		if ( $added_theme_compatibilitiy ) {
			$theme_compatibilities->set_theme_has_compatibility_adjustment( true );
		}
		...
		******************************/
 		...
 		* PageBuilder Themify
		******************************/
		add_action( 'german_market_after_init', function() {
	...
	public static function salesman_add_free_shipping_method( $free_shipping_methods ) {
		if ( is_array( self::$shipping_methods ) ) {
			foreach( self::$shipping_methods as $shipping_method_id => $shipping_method ) {
				if ( true === apply_filters( 'wgm_shipping_salesman_add_free_shipping_method_' . $shipping_method_id, true ) ) {

					$free_shipping_methods[] = $shipping_method;

					add_filter( 'marketpress_salesman_free_shipping_notice_min_amount_for_free_shipping', function ( $min_amount, $salesman ) {
						global $shipping_method_id;
						if ( ! empty( self::$shipping_methods[ $shipping_method_id ]->minimum_amount ) && ( self::$shipping_methods[ $shipping_method_id ]->minimum_amount < $min_amount ) ) {
							return self::$shipping_methods[ $shipping_method_id ]->minimum_amount;
						}
						if ( ! empty( self::$shipping_methods[ $shipping_method_id ]->free_min_amount ) && ( self::$shipping_methods[ $shipping_method_id ]->free_min_amount < $min_amount ) ) {
							return self::$shipping_methods[ $shipping_method_id ]->free_min_amount;
						}
	...
	*/
	function avia_advanced_layout_builder( $boolean, $post ) {
		...
		if ( function_exists( 'Avia_Builder' ) ) {
			if ( is_object( $post ) && isset( $post->ID ) && '' !== Avia_Builder()->get_alb_builder_status( $post->ID ) ) {
				$boolean = false;
			}
		}
	...
	*/
	function due_date() {
		...
		foreach ( $gateways as $payment_method_id => $gateway ) {
			...
			$gateway_setting = WGM_Payment_Settings::get_option( 'wgm_due_date_text', $gateway );
			if ( ! empty( $gateway_setting ) ) {
				$due_date_text 	= $gateway_setting;
			} else {
	...
	*/
	function subscriptions_gm_dont_copy_meta( $meta, $to_order, $from_order ) {
		...
		foreach ( $meta as $key => $value ) {
			...
			if ( 
				'_wp_wc_running_invoice_number' === $key || 
				'_wp_wc_running_invoice_number_date' === $key || 
				'_wp_wc_invoice_pdf_saved_html' === $key || 
				'_lexoffice_woocomerce_has_transmission' === $key || 
				'_lexoffice_woocomerce_has_transmission_invoice_api' === $key ||
				'_lexoffice_woocomerce_scheduled_for_transmission' === $key ||
				'_lexoffice_woocommerce_has_transaction_assignment' === $key ||
				'_lexoffice_invoice_number' === $key ||
				'_lexoffice_woocomerce_invoice_document_id' === $key ||
				'_sevdesk_woocomerce_has_transmission' === $key
			) {
				continue;
			}
	...
	*/
	...
	/**
	 * Wrapper for add_action regarding theme compatibilities
	 * 
	 * @since 3.26
	 ...
	 * @return void
	 */ 
	public function add_my_theme_action( $action_name, $callback, $prio = 10, $params = 1 ) {
		if ( 'off' === get_option( 'german_market_customizable_product_data_disable_theme_compatibility', 'off' ) ) {
			add_action( $action_name, $callback, $prio, $params );
		}
	}

	/**
	 * Wrapper for add_filter regarding theme compatibilities
	 * 
	 * @since 3.26
	 * @param String $action_name
	 * @param Mixed $callback
	 ...
	 * @return void
	 */ 
	public function add_my_theme_filter( $action_name, $callback, $prio = 10, $params = 1 ) {
		if ( 'off' === get_option( 'german_market_customizable_product_data_disable_theme_compatibility', 'off' ) ) {
			add_filter( $action_name, $callback, $prio, $params );
		}
	}
