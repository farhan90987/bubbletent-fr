...
/*----------------------------------------------------*/
	...
	// } );
		function second_slotcheck(date) {
			var slots = {
				isFirstSlotTaken: false,
				isSecondSlotTaken: false
			}
			console.log(wpkStartDates);

			if (typeof wpkStartDates != 'undefined' && typeof wpkEndDates != 'undefined') {
				slots.isSecondSlotTaken = wpkIsDateInArray_slots(date, wpkStartDates);
				slots.isFirstSlotTaken = wpkIsDateInArray_slots(date, wpkEndDates);
			}
			return slots;
		}
		function wpkIsDateInArray_slots(date, array) {
			//console.log("format " + date);
			return jQuery.inArray(date, array) !== -1;
	...
	$('#date-picker').daterangepicker({
			...
			var endDates = moment(end).format('YYYY-MM-DD');
			const startDate = new Date(currentDate);
			const endDate_new = new Date(endDates);

			const date = new Date(startDate.getTime());
			const dates = [];
			var flag = false;
			while (date < endDate_new) {
				dates.push(new Date(date));

				var startDate_new = new Date(date.getTime());

				var Moments_dates = moment(startDate_new).format('YYYY-MM-DD');
				var slots = second_slotcheck(Moments_dates);

				if (slots.isSecondSlotTaken && !slots.isFirstSlotTaken) {
					flag = true;
					break;
					//console.log(Moments_dates + 'second-slot-taken');
				}
				date.setDate(date.getDate() + 1);
			}
			if (flag) {
				alert("Invalid date selection, please select available dates and submit again.");
				$(".hash-custom-book-error").text('Invalid Selection');
				$('.hash-custom-book-id').hide();
			} else {
				$(".hash-custom-book-error").text('');
				$('.hash-custom-book-id').show();
			}
