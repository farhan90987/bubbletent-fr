...
class PluginExternalBlocking
{
    ...
    public function display_info_when_external_blocking(): void
    {
        ...
        if (defined('WP_HTTP_BLOCK_EXTERNAL') && \WP_HTTP_BLOCK_EXTERNAL === \true) {
            ...
            if (!defined('WP_ACCESSIBLE_HOSTS') || stristr(\WP_ACCESSIBLE_HOSTS, $host) === \false) {
				...
				<div class="error">
					<p>
					<?php 
                printf(wp_kses_post(__('<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %1$s updates. Please add %2$s to %3$s.', 'flexible-coupons-pro')), esc_html($this->plugin_info->get_plugin_name()), wp_kses_post('<strong>' . $host . '</strong>'), wp_kses_post('<code>WP_ACCESSIBLE_HOSTS</code>'));
                ?>
							</p>
				</div>
				<?php 
