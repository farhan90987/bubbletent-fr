...
namespace FlexibleCouponsProVendor\Mpdf\Image;
...
use FlexibleCouponsProVendor\Mpdf\File\StreamWrapperChecker;
use FlexibleCouponsProVendor\Mpdf\Gif\Gif;
use FlexibleCouponsProVendor\Mpdf\Language\LanguageToFontInterface;
use FlexibleCouponsProVendor\Mpdf\Language\ScriptToLanguageInterface;
use FlexibleCouponsProVendor\Mpdf\Log\Context as LogContext;
use FlexibleCouponsProVendor\Mpdf\Mpdf;
use FlexibleCouponsProVendor\Mpdf\Otl;
use FlexibleCouponsProVendor\Mpdf\RemoteContentFetcher;
use FlexibleCouponsProVendor\Mpdf\SizeConverter;
use FlexibleCouponsProVendor\Psr\Log\LoggerInterface;
class ImageProcessor implements \FlexibleCouponsProVendor\Psr\Log\LoggerAwareInterface
{
    /**
     * @var \Mpdf\Mpdf
     */
    ...
    public function getImage(&$file, $firsttime = \true, $allowvector = \true, $orig_srcpath = \false, $interpolation = \false)
    {
        ...
        if ($type === 'jpeg' || $type === 'jpg') {
            ...
            if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || $this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3)) {
                ...
                if (!function_exists('gd_info')) {
                    throw new \FlexibleCouponsProVendor\Mpdf\MpdfException(sprintf('JPG image may not use CMYK color space (%s).', $file));
                }
        ...
        if ($type === 'png') {
            ...
            if ($firsttime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
                ...
                if ($pngalpha) {
                    if ($this->mpdf->PDFA) {
                        throw new \FlexibleCouponsProVendor\Mpdf\MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
                    }
    ...
    private function gzCompress($data)
    {
        if (!function_exists('gzcompress')) {
            throw new \FlexibleCouponsProVendor\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
        }
    ...
    private function imageError($file, $firsttime, $msg)
    {
        ...
        if ($firsttime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
            throw new \FlexibleCouponsProVendor\Mpdf\MpdfImageException(sprintf('%s (%s)', $msg, substr($file, 0, 256)));
        }
