...
namespace FlexibleCouponsProVendor\Mpdf\Color;
...
class ColorConverter
{
    ...
    public function invert($c)
    {
        ...
        throw new \FlexibleCouponsProVendor\Mpdf\MpdfException('Trying to invert non-RGB color');
    }
    /**
    ...
    private function processModeColor($mode, array $cores)
    {
        ...
        switch ($mode) {
            case 'rgb':
                return [ static::MODE_RGB, (int) trim( $cores[0] ), (int) trim( $cores[1] ), (int) trim( $cores[2] ) ];
            case 'rgba':
                return [static::MODE_RGBA, (int) trim( $cores[0] ), (int) trim( $cores[1] ), (int) trim( $cores[2] ), (float) trim( $cores[3] ) * 100];
            case 'cmyk':
            case 'device-cmyk':
            ...
            case 'spot':
                ...
                if (!isset($this->mpdf->spotColors[$name])) {
                    if (isset($cores[5])) {
                        ...
                        throw new \FlexibleCouponsProVendor\Mpdf\MpdfException(sprintf('Undefined spot color "%s"', $name));
                    }
                }
    ...
    {
        if (!is_string($color)) {
            throw new \FlexibleCouponsProVendor\Mpdf\MpdfException('Invalid color input, binary color string expected');
        }
        if (strlen($color) !== 6) {
            throw new \FlexibleCouponsProVendor\Mpdf\MpdfException('Invalid color input, binary color string expected');
        }
        if (!in_array($color[0], [static::MODE_GRAYSCALE, static::MODE_SPOT, static::MODE_RGB, static::MODE_CMYK, static::MODE_RGBA, static::MODE_CMYKA])) {
            throw new \FlexibleCouponsProVendor\Mpdf\MpdfException('Invalid color input, invalid color mode in binary color string');
        }
    }
}
