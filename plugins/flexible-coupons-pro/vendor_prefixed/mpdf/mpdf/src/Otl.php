...
use FlexibleCouponsProVendor\Mpdf\Css\TextVars;
use FlexibleCouponsProVendor\Mpdf\Fonts\FontCache;
use FlexibleCouponsProVendor\Mpdf\Shaper\Indic;
use FlexibleCouponsProVendor\Mpdf\Shaper\Myanmar;
use FlexibleCouponsProVendor\Mpdf\Shaper\Sea;
use FlexibleCouponsProVendor\Mpdf\Utils\UtfString;
class Otl
{
    use Strict;
    ...
    function applyOTL($str, $useOTL)
    {
        ...
        for ($sch = 0; $sch <= $subchunk; $sch++) {
            ...
            if (!isset($this->GDEFdata[$this->fontkey]['GSUBGPOStables'])) {
                ...
                if (!$this->ttfOTLdata) {
                    throw new \FlexibleCouponsProVendor\Mpdf\MpdfException('Can\'t open file ' . $this->fontCache->tempFilename($this->fontkey . '.GSUBGPOStables.dat'));
                }
            } else {
    ...
    function _applyGSUBsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $level, $currentTag, $is_old_spec, $tagInt)
    {
        ...
        if ($Type == 1) {
            ...
            if ($SubstFormat == 1) {
                ...
                throw new \FlexibleCouponsProVendor\Mpdf\MpdfException("GSUB Lookup Type " . $Type . " Format " . $SubstFormat . " not TESTED YET.");
            }
        } elseif ($Type == 6) {
            //===========
            // Format 1:
            ...
            throw new \FlexibleCouponsProVendor\Mpdf\MpdfException("GSUB Lookup Type " . $Type . " not supported.");
        }
    }
    ...
    private function _applyGPOSsubtable($lookupID, $subtable, $ptr, $currGlyph, $currGID, $subtable_offset, $Type, $Flag, $MarkFilteringSet, $LuCoverage, $tag, $level, $is_old_spec)
    {
        ...
        if ($Type == 1) {
            ...
            //===========
            if ($PosFormat == 1) {
                throw new \FlexibleCouponsProVendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . " Format " . $PosFormat . " not TESTED YET.");
            } elseif ($PosFormat == 2) {
                $CoverageTableOffset = $subtable_offset + $this->read_ushort();
                $InputClassDefOffset = $subtable_offset + $this->read_ushort();
                ...
                throw new \FlexibleCouponsProVendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . ", Format " . $PosFormat . " not supported.");
            }
        } elseif ($Type == 8) {
            //===========
            // Format 1:
            ...
            throw new \FlexibleCouponsProVendor\Mpdf\MpdfException("GPOS Lookup Type " . $Type . " not supported.");
        }
    }
    //////////////////////////////////////////////////////////////////////////////////
    ...
    private function _getGCOMignoreString($flag, $MarkFilteringSet)
    {
        ...
        if ($flag & 0x10) {
            throw new \FlexibleCouponsProVendor\Mpdf\MpdfException("This font [" . $this->fontkey . "] contains MarkGlyphSets - Not tested yet");
            // Change also in ttfontsuni.php
            if ($MarkFilteringSet == '') {
                throw new \FlexibleCouponsProVendor\Mpdf\MpdfException("This font [" . $this->fontkey . "] contains MarkGlyphSets - but MarkFilteringSet not set");
            }
