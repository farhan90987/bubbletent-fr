 ...
 */
class Code11 extends \FlexibleCouponsProVendor\Mpdf\Barcode\AbstractBarcode implements \FlexibleCouponsProVendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
    ...
    private function init($code, $printRatio)
    {
        ...
        for ($i = 0; $i < $len; ++$i) {
            if (!isset($chr[$code[$i]])) {
                throw new \FlexibleCouponsProVendor\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE11 barcode value "%s"', $code[$i], $code));
            }
            $seq = $chr[$code[$i]];
            for ($j = 0; $j < 6; ++$j) {
                $t = $j % 2 === 0;
