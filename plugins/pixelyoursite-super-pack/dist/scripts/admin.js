jQuery(document).ready(function ($) {
    ...
    $body.on('change','.pixel_lang_check_box input'  ,function () {
        ...
        $parentInfo = $(this).parents('.pixel_info_multipixel');
    ...
    function updatePixelData($parent,key,value){
        ...
        let $input = $parent.find('> input, > textarea');
    ...
    $body.on('click','.pys_superpack_add_conditions_pixel_id',function (e) {
        ...
        const lastElement = $( '.pixel_conditions_wrap' ).find('.pixel_conditions:last');
        let $row = elem.clone()
    ...
    $('#pys_superpack_add_facebook_pixel_id').click(function (e) {
        ...
        $row.find('#facebook_is_fire_signal').attr('id','facebook_is_fire_signal_'+count)
        $row.find("label[for='facebook_is_fire_signal']").attr('for','facebook_is_fire_signal_'+count)

        $row.find('#facebook_is_fire_woo').attr('id','facebook_is_fire_woo_'+count)
        $row.find("label[for='facebook_is_fire_woo']").attr('for','facebook_is_fire_woo_'+count)

        $row.find('#facebook_is_fire_edd').attr('id','facebook_is_fire_edd_'+count)
        $row.find("label[for='facebook_is_fire_edd']").attr('for','facebook_is_fire_edd_'+count)
    ...
    $('#pys_superpack_add_ga_tracking_id').click(function (e) {
        ...
        e.preventDefault();
        let count = $(this).parents('.settings_content').find('.pixel_info').length -1
        let $row = $('#pys_superpack_ga_tracking_id').clone();
        ...
        $row.find('#pixel_ga_debug_mode').attr('id','pixel_ga_debug_mode_'+count)
        $row.find("label[for='pixel_ga_debug_mode']").attr('for','pixel_ga_debug_mode_'+count)

        $row.find('#pixel_ga_first_party_collection').attr('id','pixel_ga_first_party_collection_'+count)
        $row.find("label[for='pixel_ga_first_party_collection']").attr('for','pixel_ga_first_party_collection_'+count)

        $row.find('#ga_is_fire_signal').attr('id','ga_is_fire_signal_'+count)
        $row.find("label[for='ga_is_fire_signal']").attr('for','ga_is_fire_signal_'+count)

        $row.find('#ga_is_fire_woo').attr('id','ga_is_fire_woo_'+count)
        $row.find("label[for='ga_is_fire_woo']").attr('for','ga_is_fire_woo_'+count)

        $row.find('#ga_is_fire_edd').attr('id','ga_is_fire_edd_'+count)
        $row.find("label[for='ga_is_fire_edd']").attr('for','ga_is_fire_edd_'+count)
    ...
    $( document ).on( 'change', '.pixel_conditions .condition_select', function () {
        let value = $( this ).val(),
            ...
            $conditionWrap = $( this ).closest( '.pixel_conditions' );

        $( this ).parent().parent().nextAll( '.pys-pysselect2' ).pysselect2( 'destroy' );
        $( this ).parent().parent().nextAll( 'select,div' ).remove()
        addCondition( parent, value );
        $conditionWrap.find( '.remove-condition-wrap' ).remove();
        addRemoveButtonConditions( $conditionWrap );
    } );
    ...
    function addCondition(parent,value,defaultValue = false) {
        ...
        if(nextCondition && nextCondition.controls.type == "select_titled") {
            let select = '<div class="condition"><div class="select-standard-wrap"><select class="condition_select condition select-standard" data-name="'+nextCondition.controls.name +'" >';
        ...
        if(nextCondition && nextCondition.controls.type == "select_titled_array") {
            let select = '<div class="condition"><div class="select-standard-wrap"><select class="condition_select condition pys-pysselect2 select-standard" multiple data-name="'+nextCondition.controls.name +'" >';
        ...
        if(nextCondition && nextCondition.controls.type == "search") {
            let search = '<div class="condition_search_wrap"><div class="condition_search" data-conditional='+value+'>' +
                ...
                '<ul><li><input type="text" class="condition_search_input input-standard" /></li></ul></div></div>';
    ...
    function initConditional() {
        $('.pixel_conditions').each(function () {
            let $parent = $(this).find('.condition-group');
            ...
            addRemoveButtonConditions( $(this) );
        });
    }
    ...
    /**
     * Add remove button to conditions
     * @param $conditionWrap
     */
    function addRemoveButtonConditions ( $conditionWrap ) {
        let removeButton = $conditionWrap.parent().find('.pys-remove-condition .remove-condition-wrap').clone(),
            $parent = $conditionWrap.find('.condition-group'),
            condition = $parent.find('> .condition, > .condition_search_wrap').last();

        if ( condition.find('.remove-condition-wrap').length === 0 ) {
            condition.append(removeButton);
        }
    }

    function checkCountConditions() {
        $( ".pixel_conditions_wrap" ).each( function () {
            let max_length = 0;
            $( this ).find( '.pixel_conditions .condition-group' ).each( function () {
                let condition = $( this ).find( ' > .condition, > .condition_search_wrap' ).length;
            ...
            $( this ).attr( "class", function ( i, c ) {
                return c ? c.replace( /\bconditions-count\S*/g, "" ).trim() : "";
            } );

            $( this ).addClass( 'conditions-count-' + max_length );
        } )
    }
});
